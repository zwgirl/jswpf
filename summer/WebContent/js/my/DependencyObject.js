...
define(["dojo/_base/declare", 
        "windows/UpdateResult", "windows/RequestFlags", "threading/DispatcherObject",
        "windows/DeferredReference", "windows/Expression", "windows/OperationType",
        "windows/UncommonField", "windows/DependencyProperty", "windows/EffectiveValueEntry",
        ...
        function(declare,
		UpdateResult, RequestFlags, DispatcherObject,
		DeferredReference, Expression, OperationType,
		UncommonField, DependencyProperty, EffectiveValueEntry,
		DependencyPropertyKey, DependencyPropertyChangedEventArgs,
		AlternativeExpressionStorageCallback, LocalValueEntry,
		LocalValueEnumerator, EntryIndex){
	
	var DependencyObject= declare(DispatcherObject, {
...
//                    entry.Value = getValueCallback(this, out valueSource);
                    ...
                    entry.Value = getValueCallback(this, valueSourceObj);
...
//                Debug.Assert(!coerceWithCurrentValue, "Don't call SetCurrentValue with UnsetValue");
            ...
            var newValueHasExpressionMarker = (value == ExpressionInAlternativeStore);
            ...
            if (!newValueHasExpressionMarker) { 
                ...
                if (!isValidValue || dp.IsObjectType) { 
                    ...
                    if (newExpr != null) { 
                        ...
                        this.ValidateSources(this, newSources, newExpr); 
                    } else { 
            ...
            var currentExpr =
                    (oldEntry.HasExpressionMarker)  ? this._getExpressionCore(this, dp, metadata)
            ...
            if (handled) { 
                ...
                if (currentExpr != null) {
                    ...
                    this.UpdateSourceDependentLists(this, dp, currentSources, currentExpr, false);  // Remove
...
//        },
        ...
        //  which has been set as a DP value here.
        // 
        ProvideSelfAsInheritanceContext:function( /*object value, DependencyObject doValue*/ /*DependencyProperty dp*/ ){ 
            var doValue = argument[0] instanceof DependencyObject ? argument[0] : null;
            if (doValue == null) {
                return false;
            } 
...
// 
        ...
        CoerceValue:function(/*DependencyProperty*/ dp) {
            ...
            var entryIndex = this.LookupEntry(dp.GlobalIndex);
            var metadata = dp.GetMetadata(DependencyObjectType);
...
//                    /*ref*/ newEntry, 
        ...
        InvalidateSubProperty:function(/*DependencyProperty*/ dp) { 
        	...
        	this.NotifyPropertyChange(new DependencyPropertyChangedEventArgs(dp, dp.GetMetadata(DependencyObjectType), GetValue(dp)));
        },
...
//            /*ref*/ /*EffectiveValueEntry*/ newEntry, 
                ...
                /*bool*/                coerceWithCurrentValue, 
                /*OperationType*/       operationType){ 
        	var newEntry = newEntryObj.newEntry;
            ...
            */
            ...
            if (coerceWithCurrentValue){
                ...
                newEntryObj.newEntry = new EffectiveValueEntry(dp, FullValueSource.IsCoerced); 
            }
 
            // check for early-out opportunities: 
            //  1) the new entry is of lower priority than the current entry
            if ((newEntryObj.newEntry.BaseValueSourceInternal != BaseValueSourceInternal.Unknown) && 
                (newEntryObj.newEntry.BaseValueSourceInternal < oldEntry.BaseValueSourceInternal)){
                return 0;
            } 
            ...
            if (newEntryObj.newEntry.Value == DependencyProperty.UnsetValue){
                var fullValueSource = newEntryObj.newEntry.FullValueSource;
                isCoerceValue = (fullValueSource == FullValueSource.IsCoerced); 
                isReEvaluate = true;
 
                if (newEntryObj.newEntry.BaseValueSourceInternal == BaseValueSourceInternal.Local) {
                    isClearValue = true; 
                }
            }
            ...
            if (isReEvaluate || 
                (!newEntryObj.newEntry.IsAnimated &&
                 (oldEntry.IsAnimated || 
                 (oldEntry.IsExpression && newEntryObj.newEntry.IsExpression && (newEntryObj.newEntry.ModifiedValue.BaseValue == oldEntry.ModifiedValue.BaseValue))))) {
                // we have to compute the new value
                if (!isCoerceValue) {
                	newEntryObj.newEntry = EvaluateEffectiveValue(entryIndex, dp, metadata, oldEntry, newEntry, operationType); 
 
                    // Make sure that the call out did not cause a change to entryIndex
                    entryIndex = CheckEntryIndex(entryIndex, targetIndex); 

                    var found = (newEntryObj.newEntry.Value != DependencyProperty.UnsetValue);
                    if (!found && metadata.IsInherited) { 
                        ...
                        if (inheritanceParent != null) { 
                            ...
                            if (parentEntryIndex.Found) {
                                found = true;
                                newEntryObj.newEntry = inheritanceParent._effectiveValues[parentEntryIndex.Index].GetFlattenedEntry(RequestFlags.FullyResolved); 
                                newEntryObj.newEntry.BaseValueSourceInternal = BaseValueSourceInternal.Inherited;
                            } 
                        } 
                    }
                    ...
                    if (!found){
                    	newEntryObj.newEntry = EffectiveValueEntry.CreateDefaultValueEntry(dp, metadata.GetDefaultValue(this, dp)); 
                    }
                } else {
                    if (!oldEntry.HasModifiers) 
                    {
                    	newEntryObj.newEntry = oldEntry;
                    }
                    else 
                    {
                    	newEntryObj.newEntry = new EffectiveValueEntry(dp, oldEntry.BaseValueSourceInternal); 
                        var modifiedValue = oldEntry.ModifiedValue; 
                        var baseValue = modifiedValue.BaseValue;
                        newEntryObj.newEntry.Value = baseValue; 
                        newEntryObj.newEntry.HasExpressionMarker = oldEntry.HasExpressionMarker;

                        if (oldEntry.IsExpression) { 
                        	newEntryObj.newEntry.SetExpressionValue(modifiedValue.ExpressionValue, baseValue);
                        } 
 
                        if (oldEntry.IsAnimated) { 
                        	newEntryObj.newEntry.SetAnimatedValue(modifiedValue.AnimatedValue, baseValue);
                        }
                    }
                } 
            }
            ...
            if (coerceWithCurrentValue){ 
                var baseValue = newEntry.GetFlattenedEntry(RequestFlags.CoercionBaseValue).Value;
                
                var parOut={
                /*ref*/ entryIndex : entryIndex, 
                /*ref*/ targetIndex : targetIndex, 
                /*ref*/ newEntry : newEntryObj.newEntry,
...
//                    ref oldValue,
                ...
                newEntryObj.newEntry = parOut.newEntry;
            ...
            if (metadata.CoerceValueCallback != null &&
                !(isClearValue && newEntryObj.newEntry.FullValueSource == /*(FullValueSource)*/BaseValueSourceInternal.Default)) 
            { 
                // CALLBACK
                var baseValue = newEntryObj.newEntry.GetFlattenedEntry(RequestFlags.CoercionBaseValue).Value; 
...
//                    ref oldValue, 
            ...
            if (newEntryObj.newEntry.FullValueSource != /*(FullValueSource)*/ BaseValueSourceInternal.Default) { 
//                Debug.Assert(newEntry.BaseValueSourceInternal != BaseValueSourceInternal.Unknown, "Value source should be known at this point");
                ...
                if (newEntryObj.newEntry.BaseValueSourceInternal == BaseValueSourceInternal.Inherited) { 
                    if (DependencyObject.IsTreeWalkOperation(operationType) &&
                        (newEntryObj.newEntry.IsCoerced || newEntryObj.newEntry.IsAnimated)) {
                ...
                if (unsetValue) { 
                    ...
                    SetEffectiveValue(entryIndex, dp, metadata, newEntryObj.newEntry, oldEntry);
                } 
            } else { 
                UnsetEffectiveValue(entryIndex, dp, metadata);
            } 
            ...
            var isAValueChange = !Equals(dp, oldValue, newEntryObj.newEntry.GetFlattenedEntry(RequestFlags.FullyResolved).Value);
            ...
            if (isAValueChange ||
                (operationType == OperationType.ChangeMutableDefaultValue && oldEntry.BaseValueSourceInternal != newEntryObj.newEntry.BaseValueSourceInternal) || 
                (metadata.IsInherited && oldEntry.BaseValueSourceInternal != newEntryObj.newEntry.BaseValueSourceInternal 
...
//#endregion EventTracing
            ...
            */ 
            ...
            var newEntryNeedsContext = newEntryObj.newEntry.FullValueSource == /*(FullValueSource)*/ BaseValueSourceInternal.Local;
...
//                    !newEntry.IsAnimated, "Coerced or Animated value cannot be a deferred reference"); 
                    ...
                    dp.OwnerType != metadata.CoerceValueCallback.Method.DeclaringType) // Need 2nd check to rule out derived class callback overrides. 
                {
                    ...
                    parOut.entryIndex = CheckEntryIndex(parOut.entryIndex, parOut.targetIndex);
                }
            } 

            var coercedValue = coerceWithCurrentValue ? controlValue : coerceValueCallback(this, baseValue); 
        ...
        NotifyPropertyChange:function(/*DependencyPropertyChangedEventArgs*/ args) { 
            ...
            if (args.IsAValueChange || args.IsASubPropertyChange) {
                ...
                var objectDependentsListMap = DependentListMapField.GetValue(this);
                if (objectDependentsListMap != null) {
                    /*FrugalMap*/var dependentListMap = /*(FrugalMap)*/objectDependentsListMap;
                    var dependentList = dependentListMap[dp.GlobalIndex];
...
//#endregion EventTracing 
        ...
        /// </summary> 
        ShouldSerializeProperty:function( /*DependencyProperty*/ dp ) { 
...
//                out bool hasModifiers, out bool isExpression, out bool isAnimated, out bool isCoerced, out bool isCurrent) {
            ...
            if (entryIndex.Found) {
                var entry = _effectiveValues[entryIndex.Index]; 
...
//            Debug.Assert(!DO_Sealed, "A Sealed DO cannot be modified"); 
            ...
            var hasExpressionMarker = (value == ExpressionInAlternativeStore); 
...
//            Debug.Assert(!DO_Sealed, "A Sealed DO cannot be modified");
        ...
        OnInheritanceContextChanged:function(/*EventArgs*/ args) {
            ...
            try 
            {
                ...
                for (var i=0; i<effectiveValuesCount; i++)
                {
                    var dp = DependencyProperty.RegisteredPropertyList.List[_effectiveValues[i].PropertyIndex]; 
                    if (dp != null) { 
                        var localValue = ReadLocalValueEntry(new EntryIndex(i), dp, true /* allowDeferredReferences */); 
                        if (localValue != DependencyProperty.UnsetValue) { 
...
//        InsertEntry(/*EffectiveValueEntry*/ entry, /*uint*/ entryIndex) {
//            // For thread-safety, sealed DOs can't modify _effectiveValues. 
////            Debug.Assert(!DO_Sealed, "A Sealed DO cannot be modified");
// 
...
//                    }
// 
//                    var destEntries = new EffectiveValueEntry[newSize];
...
//            Debug.Assert(!DO_Sealed, "A Sealed DO cannot be modified");
	...
	DependencyObject.ExpressionInAlternativeStore = null;
...
//                        (DependentListMapField.GetValue(source.DependencyObject) == default(object)));
                ...
                if (!source.DependencyObject.IsSealed){ 
                    ...
                    var value = DependentListMapField.GetValue(source.DependencyObject); 
                    if (value != null){
                    ...
                    var dependentListObj = dependentListMap[source.DependencyProperty.GlobalIndex];
//                    Debug.Assert(dependentListObj != null, "dependentList should either be unset or non-null");
    ...
    /*internal static void */DependencyObject.ValidateSources = function(/*DependencyObject*/ d, 
    		/*DependencySource[]*/ newSources, /*Expression*/ expr){
        // Make sure all Sources are owned by the same thread. 
//        if (newSources != null){
//            var dispatcher = d.Dispatcher;
//            for (int i = 0; i < newSources.Length; i++){
//                var sourceDispatcher = newSources[i].DependencyObject.Dispatcher; 
//                if (sourceDispatcher != dispatcher && !(expr.SupportsUnboundSources && sourceDispatcher == null)){
//                    throw new Error("ArgumentException(SR.Get(SRID.SourcesMustBeInSameThread)"); 
//                }
//            }
//        }
    };
...
//        Debug.Assert(_getExpressionCore == null, "The 'alternative Expression storage' feature has already been registered");
        ...
        this._getExpressionCore = getExpressionCore;
    ...
    function GetExpression(/*DependencyObject*/ d, /*DependencyProperty*/ dp, /*PropertyMetadata*/ metadata) {
        ...
        if (entry.HasExpressionMarker) {
            if (this._getExpressionCore != null) { 
                return this._getExpressionCore(d, dp, metadata);
            } 
    ...
    var InheritanceContextChangedHandlersField = new UncommonField();
    ...
    DependencyObject.DependentListMapField = new UncommonField(); 

    // Optimization, to avoid calling FromSystemType too often 
    DependencyObject.DType = DependencyObjectType.FromSystemTypeInternal(typeof(DependencyObject));
