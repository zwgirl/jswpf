...
define(["dojo/_base/declare", "system/Type",
        ...
        "componentmodel/PropertyChangedEventManager"], 
...
//            GC.SuppressFinalize(this); 
	...
	var PropertyPathWorker = declare("ObjectRefArgs", IWeakEventListener, {
...
//        { 
////            var isExtendedTraceEnabled = IsExtendedTraceEnabled(TraceDataLevel.GetValue);
//
...
//            	"dpa" : null
//            };
//            this.SetPropertyInfo(this._arySVS[level].info, parObj/*out pi, out pd, out dp, out dpa*/);
// 
//            /*DependencyProperty*/var dp = parObj.dp; 
//            /*PropertyInfo*/var pi = parObj.pi;
//            /*PropertyDescriptor*/var pd = parObj.pd;
//            /*DynamicPropertyAccessor*/var dpa = parObj.dpa;
//            
...
//                else if (pd != null)
//                { 
//                    /*bool*/var indexerIsNext = (level+1 < this.SVI.Length && this.SVI[level+1].type == SourceValueType.Indexer); 
//                    value = this.Engine.GetValue(item, pd, indexerIsNext);
//                } 
//                else if (dp != null)
//                {
//                    /*DependencyObject*/var d = /*(DependencyObject)*/item;
//                    if (level != Length-1 || _host == null || this._host.TransfersDefaultValue) 
//                        value = d.GetValue(dp);
...
//                        /*int*/var index = wrapper.Value;
//                        /*IList*/var ilist = /*(IList)*/item; 
//
...
//        internal void 
        ...
        {
            for (var k=1; k<_arySVS.length; ++k) 
            {
            ...
            this.UpdateSourceValueState(Length-1, null);
        },
...
//        internal void 
        ...
        { 
            if (dp == DependencyObject.DirectDependencyProperty) 
            {
            	...
            	this.UpdateSourceValueState(_arySVS.length, null, Type.NullDataItem, isASubPropertyChange);
                return;
            } 
            ...
            { 
                if ((this._arySVS[k].info == dp) && (_arySVS[k].item == d))
                {
                    // update everything below that level
...
//        internal static bool 
        IsIndexedProperty:function(/*PropertyInfo*/ pi) 
        { 
            var result = false;
...
//                 this.Engine.RegisterForCacheChanges(newO, svs.info); 
                 ...
                 if ((newO instanceof INotifyPropertyChanged ? newO : null) != null)
                 { 
                     PropertyChangedEventManager.AddHandler(newO, new EventHandler(this, this.OnPropertyChanged), this.SVI[k].propertyName);
                 } 
             }
...
//                	this.CheckReadOnly(newO, svs.info); 
                ...
                { 
                    /*INotifyDataErrorInfo*/var indei= newO instanceof INotifyDataErrorInfo ? newO : null;
                    if (indei != null)
                    {
                        if (this.IsDynamic) 
                        {
                            ErrorsChangedEventManager.AddHandler(indei, this.OnErrorsChanged); 
                        } 

                        this._host.OnDataErrorsChanged(indei, this.SourcePropertyName); 
                    }
                }
            }
        },
...
//        internal bool 
        IsPathCurrent:function(/*object*/ rootItem) 
        {
            ...
            for (var level=0, n=Length; level<n; ++level) 
            { 
...
////        private void 
//        GetInfo:function(/*int*/ k, /*object*/ item, /*ref SourceValueState svs*/refSvs) 
//        {
...
//            var index; 
//            /*bool*/var cacheAccessor = !PropertyPath.IsParameterIndex(this.SVI[k].name, out index);
//            if (cacheAccessor) 
//            { 
//                /*AccessorInfo*/var accessorInfo = Engine.AccessorTable[this.SVI[k].type, newType, this.SVI[k].name];
...
//                    svs.item = BindingExpression.ReplaceReference(svs.item, item); 
//
//                    if (this.IsDynamic && SVI[k].type == SourceValueType.Property && svs.info instanceof DependencyProperty)
//                    {
...
//            {
//            case SourceValueType.Property: 
//                info = this._parent.ResolvePropertyName(k, item, newType, TreeContext);
//
...
//                    };
//                    
//                PropertyPath.DowncastAccessor(info, parObj/*out oldDP, out oldPI, out oldPD, out oldDOA*/); 
//                /*INotifyPropertyChanged*/var oldPC = parObj.oldPC;
//                /*DependencyProperty*/var oldDP = parObj.oldDP;
//                /*PropertyInfo*/var oldPI = parObj.oldPI;
//                /*PropertyDescriptor*/var oldPD = parObj.oldPD; 
//                /*DynamicObjectAccessor*/var oldDOA = parObj.oldDOA;
//                    
//
//                if (dp != null) 
//                { 
...
//            	var aryInfo = this._parent.ResolveIndexerParams(k, TreeContext);
//
...
// 
//            case SourceValueType.Direct: 
//                if (!(item instanceof ICollectionView) || _host == null || this._host.IsValidValue(item))
//                { 
...
//            {
//                this.Engine.AccessorTable[SVI[k].type, newType, SVI[k].name] = 
...
////        private bool 
//        ShouldConvertIndexerToProperty:function(/*object*/ item, ref string name)
//        { 
...
////        void 
//        SetPropertyInfo:function(/*object*/ info, parObj
//        		/*out PropertyInfo pi, out PropertyDescriptor pd, out DependencyProperty dp, out DynamicPropertyAccessor dpa*/)
//        { 
//        	parObj.pi = null;
//        	parObj.pd = null;
//        	parObj.dpa = null;
//        	parObj.dp = info instanceof DependencyProperty ? info : null; 
//
//            if (parObj.dp == null) 
//            { 
//            	parObj.pi = info instanceof PropertyInfo ? info : null;
//                if (parObj.pi == null) 
//                {
//                	parObj.pd = info instanceof PropertyDescriptor ? info : null;
//
//                    if (parObj.pd == null) 
//                    	parObj.dpa = info instanceof DynamicPropertyAccessor : info : null;
//                } 
//            } 
//        },
// 
...
//                	"dpa" : null
//                };
//            this.SetPropertyInfo(info, parObj/*out pi, out pd, out dp, out dpa*/);
// 
//            /*DependencyProperty*/var dp = parObj.dp; 
//            /*PropertyInfo*/var pi = parObj.pi;
//            /*PropertyDescriptor*/var pd = parObj.pd;
...
//        void 
        ...
        { 
            var result = false; 
            var item = this.GetItem(Length - 1);
...
//            this.SetPropertyInfo(_arySVS[Length-1].info, out pi, out pd, out dp, out dpa); 
            ...
            this.SetPropertyInfo(_arySVS[Length-1].info, parObj/*out pi, out pd, out dp, out dpa*/);
            ...
            var arg = (columnName == "Item" && pi != null) ? this._arySVS[Length-1].args[0] : null;
...
//                        SetPropertyInfo(GetAccessor(level), out pi, out pd, out dp, out dpa);
                        ...
                        this.SetPropertyInfo(this.GetAccessor(level), parObj/*out pi, out pd, out dp, out dpa*/);
...
//    static readonly Char[] 
    PropertyPathWorker.s_comma = ','; 
    
//    static readonly Char[] 
    PropertyPathWorker.s_dot   = '.';

//    static readonly object 
    PropertyPathWorker.NoParent = {"name" : "NoParent"}; //new NamedObject("NoParent");
//    static readonly object 
    PropertyPathWorker.AsyncRequestPending = {"name" : "AsyncRequestPending"}; //new NamedObject("AsyncRequestPending"); 
//    internal static readonly object 
    PropertyPathWorker.IListIndexOutOfRange = {"name" : "IListIndexOutOfRange"}; //new NamedObject("IListIndexOutOfRange");
