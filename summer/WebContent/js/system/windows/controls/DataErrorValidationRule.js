...
define(["dojo/_base/declare", "system/Type", "controls/ValidationRule", "controls/ValidationResult",
        "componentmodel/IDataErrorInfo", "controls/ValidationStep"], 
		function(declare, Type, ValidationRule, ValidationResult,
				IDataErrorInfo, ValidationStep){
	
	var BindingExpression = null;
	function EnsureBindingExpression(){
		if(BindingExpression == null){
			BindingExpression = using("data/BindingExpression");
		}
		
		return BindingExpression;
	}
	
	var BindingGroup = null;
	function EnsureBindingGroup(){
		if(BindingGroup == null){
			BindingGroup = using("data/BindingGroup");
		}
		
		return BindingGroup;
	}
	
	var DataErrorValidationRule = declare("DataErrorValidationRule", ValidationRule,{
		constructor:function( ){
			ValidationRule.prototype.constructor.call(this, ValidationStep.UpdatedValue, true);
		},
...
//      public override ValidationResult 
      Validate:function(/*object*/ value, /*CultureInfo*/ cultureInfo) 
      {
          ...
          if ((bindingGroup = (value instanceof EnsureBindingGroup() ? value : null)) != null) 
          { 
              // in a BindingGroup, check the item-level IDataErrorInfo for each
              // source item in the group 
              /*IList*/var items = bindingGroup.Items;
              for (var i=items.Count-1; i>=0; --i)
              {
                  /*IDataErrorInfo*/var idei = items[i] instanceof IDataErrorInfo ? items[i] : null; 
                  if (idei != null)
                  { 
                      /*string*/var error = idei.Error; 
          ...
          else if ((bindingExpr = (value instanceof EnsureBindingExpression() ? value : null)) != null) 
          { 
              // in a binding, check the error info for the binding's source
              // property 
              /*IDataErrorInfo*/var idei = bindingExpr.SourceItem;
              idei = idei instanceof IDataErrorInfo ? idei : null;
              /*string*/var name = (idei != null) ? bindingExpr.SourcePropertyName : null;
...
//                  #pragma warning restore 1634, 1691 
              ...
              throw new Error('InvalidOperationException(SR.Get(SRID.ValidationRule_UnexpectedValue, this, value)'); 

          return ValidationResult.ValidResult;
      } 
	});
	
	Object.defineProperties(DataErrorValidationRule,{
		Instance:{
			get:function(){
				if(DataErrorValidationRule._instance === undefined){
					DataErrorValidationRule._instance = new DataErrorValidationRule();
				}
				
				return DataErrorValidationRule._instance;
			}
		}
	});
	
	DataErrorValidationRule.Type = new Type("DataErrorValidationRule", DataErrorValidationRule, [ValidationRule.Type]);
	return DataErrorValidationRule;
});
