/**
 * BindingExpression
 */
/// <summary>
///  Describes a single run-time instance of data binding, binding a target
///  (element, DependencyProperty) to a source (object, property, XML node) 
/// </summary>
define(["dojo/_base/declare", "system/Type",
        "windows/DependencySource", "data/BindingExpressionBase",
        /*"data/Binding",*/ "windows/Expression", "internal.data/IDataBindEngineClient",
        "data/BindingStatusInternal", "windows/FrameworkPropertyMetadata",
        "windows/DependencyProperty", "internal.data/RelativeObjectRef",
        "internal.data/ObjectRefArgs", "data/UpdateSourceTrigger",
        "internal.data/CollectionViewSource", "data/DataSourceProvider", "internal.data/ClrBindingWorker",
        "controls/ValidationStep", "controls/DataErrorValidationRule", "data/BindingGroup"], 
        function(declare, Type,
        		DependencySource, BindingExpressionBase,
        		/*Binding,*/ Expression, IDataBindEngineClient,
        		BindingStatusInternal, FrameworkPropertyMetadata,
        		DependencyProperty, RelativeObjectRef,
        		ObjectRefArgs, UpdateSourceTrigger,
        		CollectionViewSource, DataSourceProvider, ClrBindingWorker,
        		ValidationStep, DataErrorValidationRule, BindingGroup){
	
	var Binding = null;
	function EnsureBinding(){
		if(Binding == null){
			Binding = using("data/Binding");
		}
		
		return Binding;
	}
	
	var Selector = null;
	function EnsureSelecor(){
		if(Selector == null){
			Selector = using("primitives/Selector");
		}
		
		return Selector;
	}
	
	var ComboBox = null;
	function EnsureComboBox(){
		if(ComboBox == null){
			ComboBox = using("controls/ComboBox");
		}
		
		return ComboBox;
	}

    var AttachAttempt = { First: 0, Again: 1, Last : 2 };

	var ChangingValueHelper = declare("", null,{
		constructor:function(/*BindingExpression*/ b ){
            this._bindingExpression = b; 
            b.CancelPendingTasks(); 	
		},
		
	    /*public void */Dispose:function()
        {
            this._bindingExpression.TransferValue();
//            GC.SuppressFinalize(this); 
        }
	});
	
	var Feature = BindingExpressionBase.Feature;

	ChangingValueHelper.Type = new Type("ChangingValueHelper", ChangingValueHelper, [Object.Type]);

	var BindingExpression = declare("BindingExpression", BindingExpressionBase,{
		constructor:function(/*Binding*/ binding, /*BindingExpressionBase*/ owner) {
//			base(binding, owner)
			BindingExpressionBase.prototype.constructor.call(this, binding, owner);
			
        	this.UseDefaultValueConverter = (this.ParentBinding.Converter == null); 
        	
//            WeakReference           _ctxElement;
//            object                  _dataItem; 
//            BindingWorker           _worker;
            /*Type */   this._sourceType = null; 

		},

        /// <summary> 
        ///     Notification that a Dependent that this Expression established has
        ///     been invalidated as a result of a Source invalidation 
        /// </summary>
        /// <param name="d">DependencyObject that was invalidated</param>
        /// <param name="args">Changed event args for the property that was invalidated</param>
        /*internal override void */OnPropertyInvalidation:function(/*DependencyObject*/ d, /*DependencyPropertyChangedEventArgs*/ args) 
        {
            if (d == null) 
                throw new Error('ArgumentNullException("d")'); 
            if (args == null)
                throw new Error('ArgumentNullException("args")'); 

            /*DependencyProperty*/var dp = args.Property;
            if (dp == null)
                throw new Error('InvalidOperationException(SR.Get(SRID.ArgumentPropertyMustNotBeNull, "Property", "args")'); 

            // ignore irrelevant notifications.  This test must happen before any marshalling. 
            var relevant = !this.IgnoreSourcePropertyChange; 

            if (dp == FrameworkElement.DataContextProperty && d == this.ContextElement) 
            {
                relevant = true;    // changes from context element are always relevant
            }
            else if (dp == CollectionViewSource.ViewProperty && d == this.CollectionViewSource) 
            {
                relevant = true;    // changes from the CollectionViewSource are always relevant 
            } 
            else if (dp == FrameworkElement.LanguageProperty && this.UsesLanguage && d == this.TargetElement)
            { 
                relevant = true;    // changes from target's Language are always relevant
            }
            else if (relevant)
            { 
                relevant = (this.Worker != null) && (this.Worker.UsesDependencyProperty(d, dp));
            } 
 
            if (!relevant)
                return; 

            BindingExpressionBase.prototype.OnPropertyInvalidation.call(this, d, args);
        },

        /// <summary> 
        /// Invalidate the given child expression.
        /// </summary> 
        /*internal override void */
        InvalidateChild:function(/*BindingExpressionBase*/ bindingExpression) 
        {
            // BindingExpression does not support child bindings 
        },

        /// <summary>
        /// Change the dependency sources for the given child expression. 
        /// </summary>
        /*internal override void*/ 
        ChangeSourcesForChild:function(/*BindingExpressionBase*/ bindingExpression, /*WeakDependencySource[]*/ newSources) 
        { 
            // BindingExpression does not support child bindings
        }, 

        /// <summary>
        /// Replace the given child expression with a new one.
        /// </summary> 
        /*internal override void*/ 
        ReplaceChild:function(/*BindingExpressionBase*/ bindingExpression)
        { 
            // BindingExpression does not support child bindings 
        },
 
        // register the leaf bindings with the binding group
        /*internal override void*/ 
        UpdateBindingGroup:function(/*BindingGroup*/ bg)
        {
            bg.UpdateTable(this); 
        },

        /// <summary> Force a data transfer from source to target </summary>
        /*public override void*/ 
        UpdateTarget:function()
        { 
            if (this.IsDetached)
                throw new Error("InvalidOperationException(SR.Get(SRID.BindingExpressionIsDetached)"); 
 
            if (this.Worker != null)
            { 
            	this.Worker.RefreshValue();  // calls TransferValue
            }
        },

        // Note: For Nullable types, DefaultValueConverter is created for the inner type of the Nullable. 
        //       Nullable "Drill-down" service is not provided for user provided Converters.
        /*internal void*/ 
        SetupDefaultValueConverter:function(/*Type*/ type) 
        { 
            if (!this.UseDefaultValueConverter)
                return; 

            if (this.IsInMultiBindingExpression)
            {
                this.Converter = null; 
                this._sourceType = type;
            } 
            else if (type != null && type != this._sourceType) 
            {
            	this._sourceType = type; 

                var converter = this.Engine.GetDefaultValueConverter(type,
                                    this.TargetProperty.PropertyType, this.IsReflective);
 
                if (converter == DefaultValueConverter.ValueConverterNotNeeded) 
                {
                    converter = null;     // everyone else takes null for an answer. 
                }

                this.Converter = converter;
            } 
        },

        /// <summary> 
        /// Attach the binding expression to the given target object and property.
        /// </summary> 
        /*internal override bool*/ 
        AttachOverride:function(/*DependencyObject*/ target, /*DependencyProperty*/ dp) 
        {
//        	base.AttachOverride( target, dp);
            if (!BindingExpressionBase.prototype.AttachOverride.call(this, target, dp)) 
                return false;

            // listen for InheritanceContext change (if target is mentored)
            if (this.ParentBinding.SourceReference == null || this.ParentBinding.SourceReference.UsesMentor) 
            {
                var mentor = Helper.FindMentor(target); 
                if (mentor != target) 
                {
                    InheritanceContextChangedEventManager.AddHandler(target, this.OnInheritanceContextChanged); 
                    this.UsingMentor = true;
                }
            }

            // listen for lost focus 
            if (this.IsUpdateOnLostFocus)
            { 
//                Invariant.Assert(!IsInMultiBindingExpression, "Source BindingExpressions of a MultiBindingExpression should never be UpdateOnLostFocus."); 
                LostFocusEventManager.AddHandler(target, this.OnLostFocus);
            } 

            // attach to things that need tree context.  Do it synchronously
            // if possible, otherwise post a task.  This gives the parser et al.
            // a chance to assemble the tree before we start walking it. 
            this.AttachToContext(AttachAttempt.First);
            if (this.StatusInternal == BindingStatusInternal.Unattached) 
            { 
            	this.Engine.AddTask(this, TaskOps.AttachToContext);
            } 

            return true;
        },

 
        /// <summary>
        /// Detach the binding expression from its target object and property. 
        /// </summary> 
        /*internal override void*/ 
        DetachOverride:function()
        { 
        	this.Deactivate();

        	this.DetachFromContext();
 
            // detach from target element
            var target = this.TargetElement; 
            if (target != null && this.IsUpdateOnLostFocus) 
            {
                LostFocusEventManager.RemoveHandler(target, this.OnLostFocus); 
            }

            this.ChangeValue(DependencyProperty.UnsetValue, false);
 
//            base.DetachOverride();
            BindingExpressionBase.prototype.DetachOverride.call(this);
        },
        
//        bool IDataBindEngineClient.
//        AttachToContext:function(/*bool*/ lastChance)
//        {
//            AttachToContext(lastChance ? AttachAttempt.Last : AttachAttempt.Again);
//            return (StatusInternal != BindingStatusInternal.Unattached); 
//        },
 
        // try to get information from the tree context (parent, root, etc.)
        // If everything succeeds, activate the binding. 
        // If anything fails in a way that might succeed after further layout,
        // just return (with status == Unattached).  The binding engine will try
        // again later. For hard failures, set an error status;  no more chances.
        // During the "last chance" attempt, treat all failures as "hard". 
        /*void*/ 
        AttachToContext:function(/*AttachAttempt*/ attempt)
        { 
        	if(typeof attempt == "boolean"){
        		var lastChance = attempt;
        		this.AttachToContext(lastChance ? AttachAttempt.Last : AttachAttempt.Again);
        		return (this.StatusInternal != BindingStatusInternal.Unattached); 
        	}
        	
            // if the target has been GC'd, just give up 
            var target = this.TargetElement;
            if (target == null) 
                return;     // status will be Detached

            // certain features should never be tried on the first attempt, as 
            // they certainly require at least one layout pass 
            if (attempt == AttachAttempt.First)
            { 
                // relative source with ancestor lookup
                /*ObjectRef*/var or = this.ParentBinding.SourceReference;
                if (or != null && or.TreeContextIsRequired(target))
                { 

                    return; 
                }
            } 
 
            var lastChance = (attempt == AttachAttempt.Last);

            // if the path has unresolved type names, the parser needs namesapce 
            // information to resolve them.  See XmlTypeMapper.GetTypeFromName.
            // Ignore this requirement during the last chance, and just let
            // GetTypeFromName fail if it wants to.
            if (!lastChance && this.ParentBinding.TreeContextIsRequired) 
            {
                if (target.GetValue(XmlAttributeProperties.XmlnsDictionaryProperty) == null || 
                    target.GetValue(XmlAttributeProperties.XmlNamespaceMapsProperty) == null) 
                {
                    return;
                } 
            }

            // if the binding uses a mentor, check that it exists
            var mentor = !this.UsingMentor ? target :  Helper.FindMentor(target); 
            if (mentor == null)
            { 
 
                if (lastChance)
                { 
                	this.SetStatus(BindingStatusInternal.PathError); 
                }
                return; 
            }
 
            // determine the element whose DataContext governs this BindingExpression 
            var contextElement = null;     // no context element
            var contextElementFound = true; 
            if (this.ParentBinding.SourceReference == null)
            {
                contextElement = mentor;    // usually the mentor/target element
 
                // special cases:
                // 1. if target property is DataContext, use the target's parent. 
                //      This enables <X DataContext="{Binding...}"/> and 
                // 2. if the target is ContentPresenter and the target property
                //      is Content, use the parent.  This enables 
                //          <ContentPresenter Content="{Binding...}"/>
                // 3. if target is CVS, and its inheritance context was set
                //      via DataContext, use the mentor's parent.  This enables
                //      <X.DataContext> <CollectionViewSource Source={Binding ...}/> 
                /*CollectionViewSource*/var cvs;
                if (this.TargetProperty == FrameworkElement.DataContextProperty || 
                    (this.TargetProperty == ContentPresenter.ContentProperty && 
                            target instanceof ContentPresenter) ||
                    (this.UsingMentor && 
                            (cvs = target instanceof CollectionViewSource ? target : null) != null &&
                            cvs.PropertyForInheritanceContext == FrameworkElement.DataContextProperty)
                    )
                { 
                    contextElement = FrameworkElement.GetFrameworkParent(contextElement);
                    contextElementFound = (contextElement != null); 
                } 
            }
            else 
            {
            	
                /*RelativeObjectRef*/var ror = this.ParentBinding.SourceReference;
                ror = ror instanceof RelativeObjectRef ? ror : null;
                if (ror != null && ror.ReturnsDataContext)
                { 
                	var objRefArgs = new ObjectRefArgs();
                	objRefArgs.IsTracing = traceObjectRef;
                    var o = ror.GetObject(mentor, objRefArgs);
                    contextElement = o instanceof DependencyObject ? o : null;    // ref to another element's DataContext 
                    contextElementFound = (o != DependencyProperty.UnsetValue); 
                }
            } 

            // if we need a context element, check that we found it
            if (!contextElementFound)
            { 
                if (lastChance)
                { 
                	this.SetStatus(BindingStatusInternal.PathError); 
                }
 
                return;
            } 
 
            // determine the source object, from which the path evaluation starts
            var source; 
            /*ObjectRef*/var sourceRef;

            if (contextElement != null)
            { 
                source = contextElement.GetValue(FrameworkElement.DataContextProperty);
 
                // if the data context is default null, try again later;  future 
                // layout may change the inherited value.
                // Ignore this requirement during the last chance, and just let 
                // the binding to null DataContext proceed.
                if (source == null && !lastChance && !BindingExpression.HasLocalDataContext(contextElement))
                {

                    return;
                }
            } 
            else if ((sourceRef = this.ParentBinding.SourceReference) != null)
            { 
                /*ObjectRefArgs*/
            	var args = new ObjectRefArgs();
//            		{ 
//                                        IsTracing = traceObjectRef,
//                                        ResolveNamesInTemplate = ResolveNamesInTemplate, 
//                                        };
//                args.IsTracing = traceObjectRef;
                args.ResolveNamesInTemplate = this.ResolveNamesInTemplate;
                source = sourceRef.GetDataObject(mentor, args);

                // check that the source could be found 
                if (source == DependencyProperty.UnsetValue)
                { 
                    if (lastChance) 
                    {
                    	this.SetStatus(BindingStatusInternal.PathError); 
                    }
 
                    return; 
                }
                else if (!lastChance && args.NameResolvedInOuterScope) 
                {
                    // if ElementName resolved in an outer scope, it's possible
                    // that future work might add the name to an inner scope.
                    // Schedule a task to check this. 
                	this.Engine.AddTask(this, TaskOps.VerifySourceReference);
                } 
            } 
            else
            { 
                // we get here only if we need ambient data context, but there
                // is no context element.  E.g. binding the DataContext property
                // of an element with no parent.  Just use null.
                source = null; 
            }
 
            // if we get this far, all the ingredients for a successful binding 
            // are present.  Remember what we've found and activate the binding.
            if (contextElement != null) 
            	this._ctxElement = contextElement; //new WeakReference(contextElement);

            // attach to context element
            this.ChangeWorkerSources(null, 0); 

            if (!this.UseDefaultValueConverter) 
            { 
            	this.Converter = this.ParentBinding.Converter;
 
                if (this.Converter == null)
                {
                    throw new Error('InvalidOperationException(SR.Get(SRID.MissingValueConverter)'); // report instead of throw?
                } 
            }
 
            // join the right binding group (if any) 
            this.JoinAnyBindingGroup(this.IsReflective, contextElement);
 
            this.SetStatus(BindingStatusInternal.Inactive);

            // inner BindingExpressions of PriorityBindingExpressions may not need to be activated
            if (this.IsInPriorityBindingExpression) 
                ParentPriorityBindingExpression.InvalidateChild(this);
            else    // singular BindingExpressions and those in MultiBindingExpressions should always activate 
            	this.Activate(source); 

//            GC.KeepAlive(target);   // keep target alive during activation (bug 956831) 
        },

        // Detach from things that may require tree context
        /*private void*/ 
        DetachFromContext:function() 
        {
            // detach from data source 
            if (this.HasValue(Feature.DataProvider)) 
            {
                DataChangedEventManager.RemoveHandler(this.DataProvider, this.OnDataChanged); 
            }

            if (!this.UseDefaultValueConverter)
            { 
            	this.Converter = null;
            } 
 
            if (!this.IsInBindingExpressionCollection)
            	this.ChangeSources(null); 

            if (this.UsingMentor)
            {
                var target = this.TargetElement; 
                if (target != null)
                	this.InheritanceContextChangedEventManager.RemoveHandler(target, this.OnInheritanceContextChanged); 
            } 

            this._ctxElement = null; 
        },
 
//        // Activate the BindingExpression, if necessary and possible 
//        /*internal override void*/ 
//        Activate:function()
//        { 
//            if (!this.CanActivate)
//                return;
//
//            if (this._ctxElement == null) 
//            {
//                // only activate once if there's an explicit source 
//                if (StatusInternal == BindingStatusInternal.Inactive) 
//                {
//                    var target = this.TargetElement; 
//                    if (target != null)
//                    {
//                        if (this.UsingMentor)
//                        { 
//                            target = Helper.FindMentor(target);
//                            if (target == null) 
//                            { 
//                                // mentor is not available
//                            	this.SetStatus(BindingStatusInternal.PathError); 
//                                return;
//                            } 
//                        } 
//                        
//                    	var objRefArgs = new ObjectRefArgs();
//                    	objRefArgs.ResolveNamesInTemplate = this.ResolveNamesInTemplate;
//                    	this.Activate(this.ParentBinding.SourceReference.GetDataObject(target,
//                        		objRefArgs)); 
//                    }
//                }
//            }
//            else 
//            {
//                var contextElement = this.ContextElement; 
//                if (contextElement == null) 
//                {
//                    // context element has been GC'd, or unavailable (e.g. no mentor) 
//                	this.SetStatus(BindingStatusInternal.PathError);
//                    return; 
//                } 
//
//                var item = this.GetValue(FrameworkElement.DataContextProperty); 
//
//                // if binding inactive or the data item has changed, (re-)activate
//                if (this.StatusInternal == BindingStatusInternal.Inactive || !Object.Equals(item, DataItem))
//                { 
//                	this.Activate(item);
//                } 
//            } 
//        },
// 
//        /*internal void*/ 
//        Activate:function(/*object*/ item)
//        {
//            var target = this.TargetElement;
//            if (target == null) 
//                return;
// 
//            if (item == BindingExpressionBase.DisconnectedItem) 
//            {
//                // disconnect from the former item, with the least acceptable impact 
//            	this.Disconnect();
//                return;
//            }
// 
////            var isExtendedTraceEnabled = TraceData.IsExtendedTraceEnabled(this, TraceDataLevel.Activate);
// 
//            this.Deactivate(); 
//
//            // apply magic (for CVS, DSP, etc.), unless asked not to 
//            if (!this.ParentBinding.BindsDirectlyToSource)
//            {
//                /*CollectionViewSource*/var cvs = item instanceof CollectionViewSource ? item : null;
//                this.CollectionViewSource = cvs; 
//                if (cvs != null)
//                { 
//                    item = cvs.CollectionView; 
//
//                    // the CVS is one of our implicit sources 
//                    this.ChangeWorkerSources(null, 0);
//                }
//                else
//                {
//                    // when the source is DataSourceProvider, use its data instead 
//                    item = this.DereferenceDataProvider(item);
//                } 
//            } 
//
////            this._dataItem = this.CreateReference(item); 
//            this._dataItem = item; 
// 
//            if (this.Worker == null)
//            	this.CreateWorker();
//
//            // mark the BindingExpression active 
//            this.SetStatus(BindingStatusInternal.Active);
// 
//            // attach to data item (may set error status) 
//            this.Worker.AttachDataItem();
// 
//            //
//            // initial transfer 
//            var initialTransferIsUpdate = this.IsOneWayToSource;
//            
//            var currentValue; 
//            var parObj = {"currentValue":null};
//            if (this.ShouldUpdateWithCurrentValue(target, /*out currentValue*/parObj)) 
//            {
//            	currentValue =parObj.currentValue;
//                initialTransferIsUpdate = true; 
//                this.ChangeValue(currentValue, /*notify*/false);
//                this.NeedsUpdate = true;
//            }
// 
//            if (!initialTransferIsUpdate)
//            { 
//            	var parObj = {"error":null};
//                var value = this.GetInitialValue(target, /*out error*/parObj);
//                /*ValidationError*/var error = parObj.error; 
//                var useValueFromBindingGroup = (value == BindingExpression.NullDataItem); 
//                if (!useValueFromBindingGroup)
//                {
//                	this.TransferValue(value, false);
//                } 
//
//                if (error != null) 
//                { 
//                	this.UpdateValidationError(error, useValueFromBindingGroup);
//                } 
//            }
//            else if (!this.IsInMultiBindingExpression)   // MultiBinding initiates its own update
//            {
//            	this.UpdateValue(); 
//            }
// 
////            GC.KeepAlive(target);   // keep target alive during activation (bug 956831) 
//            
//            return (this.StatusInternal != BindingStatusInternal.Unattached); 
//        },
        
        /*internal void*/ 
        Activate:function(/*object*/ item)
        {
        	if(arguments.length == 0){
        		if (!this.CanActivate)
                    return;

                if (this._ctxElement == null) 
                {
                    // only activate once if there's an explicit source 
                    if (StatusInternal == BindingStatusInternal.Inactive) 
                    {
                        var target = this.TargetElement; 
                        if (target != null)
                        {
                            if (this.UsingMentor)
                            { 
                                target = Helper.FindMentor(target);
                                if (target == null) 
                                { 
                                    // mentor is not available
                                	this.SetStatus(BindingStatusInternal.PathError); 
                                    return;
                                } 
                            } 
                            
                        	var objRefArgs = new ObjectRefArgs();
                        	objRefArgs.ResolveNamesInTemplate = this.ResolveNamesInTemplate;
                        	this.Activate(this.ParentBinding.SourceReference.GetDataObject(target,
                            		objRefArgs)); 
                        }
                    }
                }
                else 
                {
                    var contextElement = this.ContextElement; 
                    if (contextElement == null) 
                    {
                        // context element has been GC'd, or unavailable (e.g. no mentor) 
                    	this.SetStatus(BindingStatusInternal.PathError);
                        return; 
                    } 

                    var item = this.GetValue(FrameworkElement.DataContextProperty); 

                    // if binding inactive or the data item has changed, (re-)activate
                    if (this.StatusInternal == BindingStatusInternal.Inactive || !Object.Equals(item, DataItem))
                    { 
                    	this.Activate(item);
                    } 
                }
        	}else{
        		var target = this.TargetElement;
                if (target == null) 
                    return;
     
                if (item == BindingExpressionBase.DisconnectedItem) 
                {
                    // disconnect from the former item, with the least acceptable impact 
                	this.Disconnect();
                    return;
                }
     
//                var isExtendedTraceEnabled = TraceData.IsExtendedTraceEnabled(this, TraceDataLevel.Activate);
     
                this.Deactivate(); 

                // apply magic (for CVS, DSP, etc.), unless asked not to 
                if (!this.ParentBinding.BindsDirectlyToSource)
                {
                    /*CollectionViewSource*/var cvs = item instanceof CollectionViewSource ? item : null;
                    this.CollectionViewSource = cvs; 
                    if (cvs != null)
                    { 
                        item = cvs.CollectionView; 

                        // the CVS is one of our implicit sources 
                        this.ChangeWorkerSources(null, 0);
                    }
                    else
                    {
                        // when the source is DataSourceProvider, use its data instead 
                        item = this.DereferenceDataProvider(item);
                    } 
                } 

//                this._dataItem = this.CreateReference(item); 
                this._dataItem = item; 
     
                if (this.Worker == null)
                	this.CreateWorker();

                // mark the BindingExpression active 
                this.SetStatus(BindingStatusInternal.Active);
     
                // attach to data item (may set error status) 
                this.Worker.AttachDataItem();
     
                //
                // initial transfer 
                var initialTransferIsUpdate = this.IsOneWayToSource;
                
                var currentValueOut = {"currentValue":null};
                var r = this.ShouldUpdateWithCurrentValue(target, /*out currentValue*/currentValueOut);
                var currentValue = currentValueOut.currentValue;
//                if (this.ShouldUpdateWithCurrentValue(target, /*out currentValue*/parObj)) 
                if(r)
                {
                	currentValue =currentValueOut.currentValue;
                    initialTransferIsUpdate = true; 
                    this.ChangeValue(currentValue, /*notify*/false);
                    this.NeedsUpdate = true;
                }
     
                if (!initialTransferIsUpdate)
                { 
                	var errorOut = {"error":null};
                    var value = this.GetInitialValue(target, /*out error*/errorOut);
                    /*ValidationError*/var error = errorOut.error; 
                    var useValueFromBindingGroup = (value == BindingExpression.NullDataItem); 
                    if (!useValueFromBindingGroup)
                    {
                    	this.TransferValue(value, false);
                    } 

                    if (error != null) 
                    { 
                    	this.UpdateValidationError(error, useValueFromBindingGroup);
                    } 
                }
                else if (!this.IsInMultiBindingExpression)   // MultiBinding initiates its own update
                {
                	this.UpdateValue(); 
                }
     
//                GC.KeepAlive(target);   // keep target alive during activation (bug 956831) 
                
                return (this.StatusInternal != BindingStatusInternal.Unattached); 
        	}
        },
 
        // return the value for the initial data transfer.  Specifically:
        //      normal case - return UnsetValue (we'll get the value from the source object)
        //      valid proposed value - return the proposed value
        //      invalid proposed value - two subcases: 
        //          one-way binding - return default/fallback value
        //          two-way binding - retrun NullDataItem (meaning "don't transfer") 
        // 
        //      In both subcases, adopt the validation errors discovered earlier.
        //      In the two-way subcase, instead of a source-to-target transfer, we set the 
        //      the target property to the (saved) raw proposed value, as if the user
        //      had edited this property.
        /*object*/ 
        GetInitialValue:function(/*DependencyObject*/ target, /*out ValidationError error*/errorOut)
        { 
            var proposedValue;
 
            // find the binding group this binding would join if it were two-way (even if it isn't) 
            var bindingGroup = this.RootBindingExpression.FindBindingGroup(true, this.ContextElement);
            /*BindingGroup.ProposedValueEntry */var entry; 

            // get the proposed value from the binding group
            if (bindingGroup == null ||
                (entry = bindingGroup.GetProposedValueEntry(this.SourceItem, this.SourcePropertyName)) == null) 
            {
                // no proposed value 
            	errorOut.error = null; 
                proposedValue = DependencyProperty.UnsetValue;
            } 
            else
            {
                // adopt the validation error (possibly null)
            	errorOut.error = entry.ValidationError; 

                if (this.IsReflective && this.TargetProperty.IsValidValue(entry.RawValue)) 
                { 
                    // two-way binding - set the target property directly
                    target.SetValue(this.TargetProperty, entry.RawValue); 
                    proposedValue = BindingExpression.NullDataItem;
                    bindingGroup.RemoveProposedValueEntry(entry);
                }
                else if (entry.ConvertedValue == DependencyProperty.UnsetValue) 
                {
                    // invalid proposed value - use fallback/default 
                    proposedValue = this.UseFallbackValue(); 
                }
                else 
                {
                    // valid proposed value
                    proposedValue = entry.ConvertedValue;
                } 

                // if this binding didn't take over responsibility for the proposed 
                // value, add it to the list of bindings using the proposed value 
                if (proposedValue != BindingExpression.NullDataItem)
                { 
                    bindingGroup.AddBindingForProposedValue(this, this.SourceItem, this.SourcePropertyName);
                }
            }
 
            return proposedValue;
        },
 
        /*internal override void*/ 
        Deactivate:function()
        { 
            // inactive BindingExpressions don't need any more work
            if (this.StatusInternal == BindingStatusInternal.Inactive)
                return;

            // stop transfers
            this.CancelPendingTasks(); 

            // detach from data item 
            if (this.Worker != null) 
            	this.Worker.DetachDataItem();
 
            // restore default value, in case source/converter fail to provide a good value
            this.ChangeValue(BindingExpressionBase.DefaultValueObject, false);

            // don't keep a handle to old data item if the BindingExpression is inactive 
            this._dataItem = null;
 
            this.SetStatus(BindingStatusInternal.Inactive); 
        },
 
        // disconnect from the former item, with the least acceptable impact.
        // This arises when a container is removed from an ItemsControl.
        /*internal override void*/ 
        Disconnect:function()
        { 
            // disconnect this binding from its former item
//        	this._dataItem = this.CreateReference(BindingExpressionBase.DisconnectedItem);
        	this._dataItem = BindingExpressionBase.DisconnectedItem;
 
            if (this.Worker == null)
                return; 

            this.Worker.AttachDataItem();

//            base.Disconnect(); 
            BindingExpressionBase.prototype.Disconnect.call(this);
        },
 
        // if the root item is a DataSourceProvider, use its Data instead 
        // and listen for DataChanged events.  (Unless overridden explicitly
        // by the BindsDirectlyToSource property). 
        /*private object*/ 
        DereferenceDataProvider:function(/*object*/ item)
        {
            /*DataSourceProvider*/var newDataProvider = item instanceof DataSourceProvider ? item : null;
            /*DataSourceProvider*/var oldDataProvider = this.DataProvider; 
            if (newDataProvider != oldDataProvider)
            { 
                // we have a new data provider - retarget the event handler 
                if (oldDataProvider != null)
                { 
//                    DataChangedEventManager.RemoveHandler(oldDataProvider, OnDataChanged);
                	oldDataProvider.DataChanged.Remove(new EventHandler(this, this.OnDataChanged));
                }

                this.DataProvider = newDataProvider; 
                oldDataProvider = newDataProvider;

                if (newDataProvider != null) 
                { 
//                    DataChangedEventManager.AddHandler(newDataProvider, OnDataChanged);
                	oldDataProvider.DataChanged.Combine(new EventHandler(this, this.OnDataChanged));
                    newDataProvider.InitialLoad(); 
                }
            }

            return (oldDataProvider != null) ? oldDataProvider.Data : item; 
        },
 
        // Return the object from which the given value was obtained, if possible 
        /*internal override object*/ 
        GetSourceItem:function(/*object*/ newValue)
        { 
            return this.SourceItem;
        },
 
        /*private void*/ 
        CreateWorker:function()
        { 
//            Invariant.Assert(Worker == null, "duplicate call to CreateWorker");

            this._worker = new ClrBindingWorker(this, this.Engine);
        },

        // worker calls here if it changes its dependency sources 
        // n is the number of real entries in newWorkerSources (which may be longer) 
        /*internal void */
        ChangeWorkerSources:function(/*WeakDependencySource[]*/ newWorkerSources, /*int*/ n)
        { 
            var offset = 0;
            var size = n;

            // create the new sources array, and add the context and CollectionViewSource elements 
            var contextElement = this.ContextElement;
            /*CollectionViewSource*/var cvs = this.CollectionViewSource; 
            var usesLanguage = this.UsesLanguage; 

            if (contextElement != null) 
                ++size;
            if (cvs != null)
                ++size;
            if (usesLanguage) 
                ++size;
 
//            WeakDependencySource[] newSources = (size > 0) ? new WeakDependencySource[size] : null; 
            var newSources = [];

            if (contextElement != null) 
            {
//                newSources[offset++] = new WeakDependencySource(_ctxElement, FrameworkElement.DataContextProperty);
            	newSources[offset++] = new DependencySource(this._ctxElement, FrameworkElement.DataContextProperty);
            }
 
            if (cvs != null)
            { 
//                var wr = GetValue(Feature.CollectionViewSource, null); 
//                newSources[offset++] = 
//                    (wr != null) ? new WeakDependencySource(wr, CollectionViewSource.ViewProperty) 
//                                 : new WeakDependencySource(cvs, CollectionViewSource.ViewProperty);
            	var cvs = GetValue(Feature.CollectionViewSource, null);
                newSources[offset++] = new DependencySource(cvs, CollectionViewSource.ViewProperty);

            }

//            if (usesLanguage) 
//            {
////                newSources[offset++] = new WeakDependencySource(TargetElementReference, FrameworkElement.LanguageProperty); 
//            	newSources[offset++] = new DependencySource(this.TargetElementReference, FrameworkElement.LanguageProperty); 
//            } 

//            // add the worker's sources 
//            if (n > 0)
//                Array.Copy(newWorkerSources, 0, newSources, offset, n);
            if(n>0){
            	for(var i =0; i<newWorkerSources.length; i++){
            		newSources[offset++] = newWorkerSources[i];
            	}
            }

            // tell the property engine 
            this.ChangeSources(newSources);
        },

        // transfer a value from the source to the target
        /*void*/ 
//        TransferValue:function() 
//        {
//        	this.TransferValue(DependencyProperty.UnsetValue, false); 
//        },

        // transfer a value from the source to the target 
        /*internal void*/ 
        TransferValue:function(/*object*/ newValue, /*bool*/ isASubPropertyChange)
        {
        	if(arguments.length == 0){
        		this.TransferValue(DependencyProperty.UnsetValue, false);
        	}
            // if the target element has been GC'd, do nothing
            var target = this.TargetElement; 
            if (target == null)
                return; 
 
            // if the BindingExpression hasn't activated, do nothing
            if (this.Worker == null) 
                return;

            var targetType = this.GetEffectiveTargetType();
            var implicitConverter = null; 
//            var isExtendedTraceEnabled = TraceData.IsExtendedTraceEnabled(this, TraceDataLevel.Transfer);
 
            // clear the Pending flag before actually doing the transfer.  That way if another 
            // thread sets the flag, we'll schedule another transfer.  This might do more
            // transfers than absolutely necessary, but it guarantees that we'll eventually pick 
            // up the value from the last change.
            this.IsTransferPending = false;
            this.IsInTransfer = true;
 
            this.UsingFallbackValue = false;
 
            // get the raw value from the source 
            var value = (newValue == DependencyProperty.UnsetValue) ? this.Worker.RawValue() : newValue;
 
            this.UpdateNotifyDataErrors(value);

            // apply any necessary conversions
            if (value != DependencyProperty.UnsetValue)
            { 
//            #if !TargetNullValueBC  //BreakingChange
//                bool doNotSetStatus = false; 
//            #endif 

                if (!this.UseDefaultValueConverter) 
                {
                    // if there's a user-defined converter, call it without catching
                    // exceptions (bug 992237).  It can return DependencyProperty.UnsetValue
                    // to indicate a failure to convert. 
                    value = this.Converter.Convert(value, targetType, this.ParentBinding.ConverterParameter/*, this.GetCulture()*/);
 
                    if (this.IsDetached) 
                    {
                        // user code detached the binding.  Give up. 
                        return;
                    }

                    // chain in a default value converter if the returned value's type is not compatible with the targetType
                    if (   ((value != null) && (value != EnsureBinding().DoNothing) && (value != DependencyProperty.UnsetValue)) 
                        && !targetType.IsAssignableFrom(value.GetType()))
                    { 
                        // the dynamic converter is shared between Transfer and Update directions 
                        // once instantiated, DefaultValueConverters are kept in a lookup table, making swapping
                        // default value converters in the DynamicValueConverter reasonably fast 
                        implicitConverter = this.DynamicConverter;
                    }
                }
                else 
                {
                    // if there's no user-defined converter, use the default converter (if any) 
                    // we chose earlier (in SetupDefaultValueConverter) 
                    implicitConverter = this.Converter;
                } 

                // apply an implicit conversion, if needed.  This can be
                //  a) null conversion
                //  b) string formatting 
                //  c) type conversion
                if ((value != EnsureBinding().DoNothing) && (value != DependencyProperty.UnsetValue)) 
                { 
                    // ultimately, TargetNullValue should get assigned implicitly,
                    // even if the user doesn't declare it.  We can't do this yet because 
                    // of back-compat.  I wrote it both ways, and #if'd out the breaking
                    // change.
//                #if TargetNullValueBC   //BreakingChange
//                    if (IsNullValue(value)) 
//                #else
                    if (this.EffectiveTargetNullValue != DependencyProperty.UnsetValue && 
                    		this.IsNullValue(value)) 
//                #endif
                    { 
                        value = this.EffectiveTargetNullValue;

                    }
//                #if !TargetNullValueBC   //BreakingChange
//                    // For DBNull, unless there's a user converter, we handle it here
//                    else if ((value == DBNull.Value) && (Converter == null || UseDefaultValueConverter)) 
//                    {
//                        if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>)) 
//                        { 
//                            value = null;
//                        } 
//                        else
//                        {
//                            value = DependencyProperty.UnsetValue;
//                            // The 3.5 code failed to set the status to UpdateTargetError in this 
//                            // case.  It's a bug, but we have to maintain the buggy behavior for
//                            // back-compat. 
//                            doNotSetStatus = true; 
//                        }
//
//                    } 
//                #endif
                    else if (implicitConverter != null || this.EffectiveStringFormat != null)
                    {
                        // call a DefaultValueConverter: 
                        // NOTE:
                        // here we pass in the TargetElement that is expected by our default value converters; 
                        // this does violate the general rule that value converters should be stateless 
                        // and must not be aware of e.g. their target element.
                        // Our DefaultValueConverters are all internal and only use this target element 
                        // to determine a BaseUri for their TypeConverters
                        // -> hence a reluctant exeption of above rule
                        value = this.ConvertHelper(implicitConverter, value, targetType, this.TargetElement/*, this.GetCulture()*/);
                    }
                } 

                if ( 
//            #if !TargetNullValueBC   //BreakingChange 
//                    !doNotSetStatus &&
//            #endif 
                    value == DependencyProperty.UnsetValue)
                {
                	this.SetStatus(BindingStatusInternal.UpdateTargetError);
                } 
            }
 
            // the special value DoNothing means no error, but no data transfer 
            if (value == EnsureBinding().DoNothing){
//                goto Done; 
            	this.IsInTransfer = false;
            	return;
            }

            // if the value isn't acceptable to the target property, don't use it
            // (in MultiBinding, the value will go through the multi-converter, so
            // it's too early to make this judgment) 
            if (!this.IsInMultiBindingExpression && value != BindingExpression.IgnoreDefaultValue &&
                value != DependencyProperty.UnsetValue && !this.TargetProperty.IsValidValue(value)) 
            { 

                value = DependencyProperty.UnsetValue;
 
                if (this.StatusInternal == BindingStatusInternal.Active)
                	this.SetStatus(BindingStatusInternal.UpdateTargetError); 
            } 

            // If we haven't obtained a value yet, 
            // use the fallback value.  This could happen when the currency
            // has moved off the end of the collection, e.g.
            if (value == DependencyProperty.UnsetValue)
            { 
                value = this.UseFallbackValue();
            }
 
            // Ignore a default source value by setting the value to NoValue; 
            // this causes the property engine to obtain the value elsewhere
            if (value == BindingExpression.IgnoreDefaultValue) 
            {
                value = Expression.NoValue;
            }

            // if this is a re-transfer after a source update and the value 
            // hasn't changed, don't do any more work.
            var realTransfer = !(this.IsInUpdate && Object.Equals(value, this.Value)); 
 
            if (realTransfer)
            { 
                // update the cached value
            	this.ChangeValue(value, true);

                // push the new value through the property engine 
            	this.Invalidate(isASubPropertyChange);
 
                // evaluate "forward" validation rules 
            	this.ValidateOnTargetUpdated();
            } 

            // after updating all the state (value, validation), mark the binding clean
            this.Clean();
 
            if (realTransfer)
            { 
            	this.OnTargetUpdated(); 
            }
 
//        Done:
        	this.IsInTransfer = false;

//            GC.KeepAlive(target);   // keep target alive during transfer (bug 956831) 
        },
 
        // run the validation rules marked as ValidateOnTargetUpdated 
        /*private void*/ 
        ValidateOnTargetUpdated:function()
        { 
            // update the validation state
            /*ValidationError*/var validationError = null;

            /*Collection<ValidationRule>*/var validationRules = this.ParentBinding.ValidationRulesInternal; 
            /*CultureInfo*/var culture = null;
 
            var needDataErrorRule = this.ParentBinding.ValidatesOnDataErrors; 

            if (validationRules != null) 
            {
                // these may be needed by several rules, but only compute them once
                var rawValue = DependencyProperty.UnsetValue;
                var itemValue = DependencyProperty.UnsetValue; 

                var validationRule = null;
                for(var i=0; i<validationRules.Count; i++) 
                { 
                	validationRule = validationRules.Get(i);
                    if (validationRule.ValidatesOnTargetUpdated)
                    { 
                        if (validationRule instanceof DataErrorValidationRule)
                        {
                            needDataErrorRule = false;
                        } 

                        var value; 
 
                        switch (validationRule.ValidationStep)
                        { 
                            case ValidationStep.RawProposedValue:
                                if (rawValue == DependencyProperty.UnsetValue)
                                {
                                    rawValue = this.GetRawProposedValue(); 
                                }
                                value = rawValue; 
                                break; 
                            case ValidationStep.ConvertedProposedValue:
                            case ValidationStep.UpdatedValue: 
                            case ValidationStep.CommittedValue:
                                if (itemValue == DependencyProperty.UnsetValue)
                                {
                                    itemValue = this.Worker.RawValue(); 
                                }
                                value = itemValue; 
                                break; 
                            default:
                                throw new Error('InvalidOperationException(SR.Get(SRID.ValidationRule_UnknownStep, validationRule.ValidationStep, validationRule)'); 
                        }

//                        // lazy-fetch culture (avoids exception when target DP is Language)
//                        if (culture == null) 
//                        {
//                            culture = this.GetCulture(); 
//                        } 

                        validationError = this.RunValidationRule(validationRule, value/*, culture*/); 
                        if (validationError != null)
                            break;
                    }
                } 
            }
 
            if (needDataErrorRule && validationError == null) 
            {
//                // lazy-fetch culture (avoids exception when target DP is Language) 
//                if (culture == null)
//                {
//                    culture = this.GetCulture();
//                } 

                validationError = this.RunValidationRule(DataErrorValidationRule.Instance, this/*, culture*/); 
            } 

            this.UpdateValidationError(validationError); 
        },

        /*ValidationError*/ 
        RunValidationRule:function(/*ValidationRule*/ validationRule, /*object*/ value, /*CultureInfo*/ culture)
        { 
            /*ValidationError*/var error;
 
            /*ValidationResult*/var validationResult = validationRule.Validate(value, culture, this); 

            if (validationResult.IsValid) 
            {
                error = null;
            }
            else 
            {
                error = new ValidationError(validationRule, this, validationResult.ErrorContent, null);
            } 
 
            return error;
        }, 

        /*private object*/ 
        ConvertHelper:function(/*IValueConverter*/ converter, /*object*/ value, /*Type*/ targetType, /*object*/ parameter, /*CultureInfo*/ culture)
        {
            // use the StringFormat (if appropriate) in preference to the default converter 
            var stringFormat = this.EffectiveStringFormat;
//            Invariant.Assert(converter != null || stringFormat != null); 
 
            // PreSharp uses message numbers that the C# compiler doesn't know about.
            // Disable the C# complaints, per the PreSharp documentation. 
//            #pragma warning disable 1634, 1691
//
//            // PreSharp complains about catching NullReference (and other) exceptions.
//            // It doesn't recognize that IsCritical[Application]Exception() handles these correctly. 
//            #pragma warning disable 56500
 
            var convertedValue = null; 
            try
            { 
                if (stringFormat != null)
                {
                    convertedValue = String.Format(culture, stringFormat, value);
                } 
                else
                { 
                    convertedValue = converter.Convert(value, targetType, parameter, culture); 
                }
            } 

            // Catch all exceptions.  There is no app code on the stack,
            // so the exception isn't actionable by the app.
            // Yet we don't want to crash the app. 
            catch (ex)
            { 
//                // the DefaultValueConverter can end up calling BaseUriHelper.GetBaseUri() 
//                // which can raise SecurityException if the app does not have the right FileIO privileges
//                if (CriticalExceptions.IsCriticalApplicationException(ex)) 
//                    throw;
                convertedValue = DependencyProperty.UnsetValue; 
            }
 
            return convertedValue;
        }, 
 
        /*private object*/ 
        ConvertBackHelper:function(/*IValueConverter*/ converter,
                                         /*object*/ value, 
                                         /*Type*/ sourceType,
                                         /*object*/ parameter,
                                         /*CultureInfo*/ culture)
        { 
//            Invariant.Assert(converter != null);
 
            // PreSharp uses message numbers that the C# compiler doesn't know about. 
            // Disable the C# complaints, per the PreSharp documentation.
//            #pragma warning disable 1634, 1691 
//
//            // PreSharp complains about catching NullReference (and other) exceptions.
//            // It doesn't recognize that IsCritical[Application]Exception() handles these correctly.
//            #pragma warning disable 56500 

            var convertedValue = null; 
            try 
            {
                convertedValue = converter.ConvertBack(value, sourceType, parameter, culture); 
            }

            // Catch all exceptions.  There is no app code on the stack,
            // so the exception isn't actionable by the app. 
            // Yet we don't want to crash the app.
            catch (/*Exception*/ ex) 
            { 
//                // the DefaultValueConverter can end up calling BaseUriHelper.GetBaseUri()
//                // which can raise SecurityException if the app does not have the right FileIO privileges 
//                ex = CriticalExceptions.Unwrap(ex);
//                if (CriticalExceptions.IsCriticalApplicationException(ex))
//                    throw;
 
//                if (TraceData.IsEnabled)
//                { 
//                    TraceData.Trace(TraceEventType.Error, 
//                        TraceData.BadConverterForUpdate(
//                            AvTrace.ToStringHelper(Value), 
//                            AvTrace.TypeName(value)),
//                        this, ex);
//                }
 
                this.ProcessException(ex, this.ValidatesOnExceptions);
                convertedValue = DependencyProperty.UnsetValue; 
            } 
//            catch(ex) // non CLS compliant exception
//            { 
//                convertedValue = DependencyProperty.UnsetValue; 
//            }

            return convertedValue; 
        },

        /*internal void*/ 
        ScheduleTransfer:function(/*bool*/ isASubPropertyChange) 
        {
            if (isASubPropertyChange && this.Converter != null)
            {
                // a converter doesn't care about sub-property changes 
                isASubPropertyChange = false;
            } 
 
            this.TransferValue(DependencyProperty.UnsetValue, isASubPropertyChange);
        }, 


        /*void*/ 
        OnTargetUpdated:function()
        { 
            if (this.NotifyOnTargetUpdated)
            { 
                var target = this.TargetElement; 
                if (target != null)
                { 
                    if (    !this.IsInMultiBindingExpression           // not an inner BindingExpression
                        &&  (   !this.IsInPriorityBindingExpression
                            ||  this == this.ParentPriorityBindingExpression.ActiveBindingExpression)    // in ProrityBinding, and either active
                            ||  (this.IsAttaching && (this.StatusInternal==BindingStatusInternal.Active || this.UsingFallbackValue) // or about to become active 
                            )
                        ) 
                    { 
                        // while attaching a normal (not style-defined) BindingExpression,
                        // we must defer raising the event until after the 
                        // property has been invalidated, so that the event handler
                        // gets the right value if it asks (bug 1036862)
                        if (this.IsAttaching && this.RootBindingExpression == target.ReadLocalValue(this.TargetProperty))
                        { 
                        	this.Engine.AddTask(this, TaskOps.RaiseTargetUpdatedEvent);
                        } 
                        else 
                        {
                        	this.OnTargetUpdated(target, this.TargetProperty); 
                        }
                    }
                }
            } 
        },
 
        /*void*/ 
        OnSourceUpdated:function() 
        {
            if (this.NotifyOnSourceUpdated) 
            {
                var target = this.TargetElement;
                if (target != null)
                { 
                    if (    !this.IsInMultiBindingExpression           // not an inner BindingExpression
                        &&  (   !this.IsInPriorityBindingExpression 
                            ||  this == this.ParentPriorityBindingExpression.ActiveBindingExpression))   // not an inactive BindingExpression 
                    {
                    	this.OnSourceUpdated(target, this.TargetProperty); 
                    }
                }
            }
        }, 

 
        // transfer a value from target to source 
        /*internal override bool*/ 
        UpdateOverride:function()
        { 
            // various reasons not to update:
            if (   !this.NeedsUpdate                     // nothing to do
                || !this.IsReflective                    // no update desired
                || this.IsInTransfer                     // in a transfer 
                || this.Worker == null                   // not activated
                || !this.Worker.CanUpdate                // no source (currency moved off end) 
                ) 
                return true;

            return this.UpdateValue();
        },

        /*internal override object*/ 
        ConvertProposedValue:function(/*object*/ value) 
        {
            var rawValue = value; 
            var sourceType = Worker.SourcePropertyType; 
            /*IValueConverter*/var implicitConverter = null;
//            var culture = GetCulture(); 

            // apply user-defined converter
            if (this.Converter != null)
            { 
                if (!this.UseDefaultValueConverter)
                { 
                    // if there's a user-defined converter, call it without catching 
                    // exceptions (bug 992237).  It can return DependencyProperty.UnsetValue
                    // to indicate a failure to convert. 
                    value = this.Converter.ConvertBack(value, sourceType, ParentBinding.ConverterParameter/*, culture*/);

                    if (this.IsDetached)
                    { 
                        // user code detached the binding.  Give up.
                        return EnsureBinding().DoNothing; 
                    } 
 
                    // chain in a default value converter if the returned value's type is not compatible with the sourceType
                    if (value != EnsureBinding().DoNothing && value != DependencyProperty.UnsetValue && 
                        !this.IsValidValueForUpdate(value, sourceType))
                    {
                        // the dynamic converter is shared between Transfer and Update directions
                        // once instantiated, DefaultValueConverters are kept in a lookup table, making swapping 
                        // default value converters reasonably fast
                        implicitConverter = this.DynamicConverter; 
                    } 
                }
                else 
                {
                    implicitConverter = this.Converter;
                }
            } 

            // apply an implicit conversion, if needed.  This can be 
            //  a) null conversion 
            //  b) type conversion
            if (value != EnsureBinding().DoNothing && value != DependencyProperty.UnsetValue) 
            {
                if (this.IsNullValue(value))
                {
                    if (value == null || !this.IsValidValueForUpdate(value, sourceType)) 
                    {
                        if (this.Worker.IsDBNullValidForUpdate) 
                        { 
                            value = DBNull.Value;
                        } 
                        else
                        {
                            value = NullValueForType(sourceType);
                        } 
                    }
                } 
                else if (implicitConverter != null) 
                {
                    // here we pass in the TargetElement, see NOTE of caution in TransferValue() why this is ok 
                    value = ConvertBackHelper(implicitConverter, value, sourceType, this.TargetElement, culture);
                }
            }

            // if the conversion failed, signal a validation error
            if (value == DependencyProperty.UnsetValue) 
            {
                if (this.ValidationError == null) 
                { 
                    var validationError = new ValidationError(ConversionValidationRule.Instance, this, 
                    		'SR.Get(SRID.Validation_ConversionFailed, rawValue)', null);
                    this.UpdateValidationError(validationError); 
                }
            }

            return value; 
        },
 
        /// <summary> 
        /// Get the converted proposed value and inform the binding group
        /// <summary> 
        /*internal override bool*/ 
        ObtainConvertedProposedValue:function(/*BindingGroup*/ bindingGroup)
        {
            var result = true;
            var value; 

            if (this.NeedsUpdate) 
            { 
                value = bindingGroup.GetValue(this);
                if (value != DependencyProperty.UnsetValue) 
                {
                    value = this.ConvertProposedValue(value);
                    if (value == DependencyProperty.UnsetValue)
                    { 
                        result = false;
                    } 
                } 
            }
            else 
            {
                value = this.BindingGroup.DeferredSourceValue;
            }
 
            bindingGroup.SetValue(this, value);
            return result; 
        },
        
        /// <summary> Send the current value back to the source </summary> 
        /// <remarks> Does nothing when binding's Mode is not TwoWay or OneWayToSource </remarks>
        /*public override void*/ 
        UpdateSource:function()
        {
            if (this.IsDetached) 
                throw new Error("InvalidOperationException(SR.Get(SRID.BindingExpressionIsDetached)");
 
            this.NeedsUpdate = true;     // force update 
            this.Update();               // update synchronously
        }, 
        
        /// <summary>
        /// Update the source value and inform the binding group 
        /// <summary>
        /*internal override bool*/ 
        UpdateSource:function(/*BindingGroup*/ bindingGroup) 
        { 
        	var result = true;
        	if (this.NeedsUpdate) 
        	{
        		var value = bindingGroup.GetValue(this);
        		value = this.UpdateSource(value);
        		bindingGroup.SetValue(this, value); 
        		if (value == DependencyProperty.UnsetValue)
        		{ 
        			result = false; 
        		}
        	} 
        	return result;
        },

        /*internal override object*/ 
        UpdateSource:function(/*object*/ value) 
        {
        	if(arguments.length == 0){
                if (this.IsDetached) 
                    throw new Error("InvalidOperationException(SR.Get(SRID.BindingExpressionIsDetached)");
     
                this.NeedsUpdate = true;     // force update 
                this.Update();  
        	}else if(arguments.length == 1){
        		if(value instanceof BindingGroup){
                	var result = true;
                	if (this.NeedsUpdate) 
                	{
                		var value = bindingGroup.GetValue(this);
                		value = this.UpdateSource(value);
                		bindingGroup.SetValue(this, value); 
                		if (value == DependencyProperty.UnsetValue)
                		{ 
                			result = false; 
                		}
                	} 
                	return result;
        		}else{
        			// If there is a failure to convert, then Update failed.
                    if (value == DependencyProperty.UnsetValue)
                    { 
                    	this.SetStatus(BindingStatusInternal.UpdateSourceError);
                    } 
         
                    if (value == EnsureBinding().DoNothing || value == DependencyProperty.UnsetValue ||
                    		this.ShouldIgnoreUpdate()) 
                    {
                        return value;
                    }
         
                    try
                    { 
                    	this.BeginSourceUpdate(); 
                    	this.Worker.UpdateValue(value);
                    } 

                    // Catch all exceptions.  There is no app code on the stack,
                    // so the exception isn't actionable by the app.
                    // Yet we don't want to crash the app. 
                    catch (/*Exception*/ ex)
                    { 
                    	this.ProcessException(ex, (this.ValidatesOnExceptions || this.BindingGroup != null));
                    	this.SetStatus(BindingStatusInternal.UpdateSourceError); 
                        value = DependencyProperty.UnsetValue; 
                    } finally { 
                    	this.EndSourceUpdate();
                    }
         
                    this.OnSourceUpdated(); 

                    return value; 
        		}
        	}
        },
        
//        /// <summary> Send the current value back to the source </summary> 
//        /// <remarks> Does nothing when binding's Mode is not TwoWay or OneWayToSource </remarks>
//        /*public override void*/ 
//        UpdateSource:function()
//        {
//            if (this.IsDetached) 
//                throw new Error("InvalidOperationException(SR.Get(SRID.BindingExpressionIsDetached)");
// 
//            this.NeedsUpdate = true;     // force update 
//            this.Update();               // update synchronously
//        }, 
//        
//        /// <summary>
//        /// Update the source value and inform the binding group 
//        /// <summary>
//        /*internal override bool*/ 
//        UpdateSource:function(/*BindingGroup*/ bindingGroup) 
//        { 
//        	var result = true;
//        	if (this.NeedsUpdate) 
//        	{
//        		var value = bindingGroup.GetValue(this);
//        		value = this.UpdateSource(value);
//        		bindingGroup.SetValue(this, value); 
//        		if (value == DependencyProperty.UnsetValue)
//        		{ 
//        			result = false; 
//        		}
//        	} 
//        	return result;
//        },
//
//        /*internal override object*/ 
//        UpdateSource:function(/*object*/ value) 
//        {
//            // If there is a failure to convert, then Update failed.
//            if (value == DependencyProperty.UnsetValue)
//            { 
//            	this.SetStatus(BindingStatusInternal.UpdateSourceError);
//            } 
// 
//            if (value == EnsureBinding().DoNothing || value == DependencyProperty.UnsetValue ||
//            		this.ShouldIgnoreUpdate()) 
//            {
//                return value;
//            }
// 
//            try
//            { 
//            	this.BeginSourceUpdate(); 
//            	this.Worker.UpdateValue(value);
//            } 
//
//            // Catch all exceptions.  There is no app code on the stack,
//            // so the exception isn't actionable by the app.
//            // Yet we don't want to crash the app. 
//            catch (/*Exception*/ ex)
//            { 
//            	this.ProcessException(ex, (this.ValidatesOnExceptions || this.BindingGroup != null));
//            	this.SetStatus(BindingStatusInternal.UpdateSourceError); 
//                value = DependencyProperty.UnsetValue; 
//            } finally { 
//            	this.EndSourceUpdate();
//            }
// 
//            this.OnSourceUpdated(); 
//
//            return value; 
//        },

        /// <summary> 
        /// Store the value in the binding group
        /// </summary> 
        /*internal override void*/ 
        StoreValueInBindingGroup:function(/*object*/ value, /*BindingGroup*/ bindingGroup) 
        {
            bindingGroup.SetValue(this, value); 
        },

        /// <summary>
        /// Run validation rules for the given step 
        /// <summary>
        /*internal override bool*/ 
        Validate:function(/*object*/ value, /*ValidationStep*/ validationStep) 
        { 
            // run rules attached to this binding
//            var result = base.Validate(value, validationStep); 
            var result = BindingExpressionBase.prototype.Validate.call(this, value, validationStep); 

            if (validationStep == ValidationStep.UpdatedValue)
            {
                if (result && this.ParentBinding.ValidatesOnDataErrors) 
                {
                    // remember the old validation error, if it came from the implicit DataError rule 
                    /*ValidationError*/var oldValidationError = this.GetValidationErrors(validationStep); 
                    if (oldValidationError != null &&
                        oldValidationError.RuleInError != DataErrorValidationRule.Instance) 
                    {
                        oldValidationError = null;
                    }
 
                    // run the DataError rule, even though it doesn't appear in the
                    // ValidationRules collection 
                    /*ValidationError*/var error = this.RunValidationRule(DataErrorValidationRule.Instance, this/*, GetCulture()*/); 

                    if (error != null) 
                    {
                    	this.UpdateValidationError(error);
                        result = false;
                    } 
                    else if (oldValidationError != null)
                    { 
                        // the implicit rule is now valid - clear the old error 
                    	this.UpdateValidationError(null);
                    } 
                }
            }
            else if (validationStep == ValidationStep.CommittedValue)
            { 
                if (result)
                { 
                	this.NeedsValidation = false; 
                }
            } 

            return result;
        },
 
        /// <summary>
        /// Run validation rules for the given step, and inform the binding group 
        /// <summary> 
        /*internal override bool*/ 
        CheckValidationRules:function(/*BindingGroup*/ bindingGroup, /*ValidationStep*/ validationStep)
        { 
            if (!this.NeedsValidation)
                return true;

            var value; 
            switch (validationStep)
            { 
                case ValidationStep.RawProposedValue: 
                    value = this.GetRawProposedValue();
                    break; 
                case ValidationStep.ConvertedProposedValue:
                    value = bindingGroup.GetValue(this);
                    break;
                case ValidationStep.UpdatedValue: 
                case ValidationStep.CommittedValue:
                    value = this; 
                    break; 
                default:
                    throw new Error('InvalidOperationException(SR.Get(SRID.ValidationRule_UnknownStep, validationStep, bindingGroup)'); 
            }

            return this.Validate(value, validationStep);
        },

        /// <summary> 
        /// Get the proposed value(s) that would be written to the source(s), applying 
        /// conversion and checking UI-side validation rules.
        /// </summary> 
        /*internal override bool*/ 
        ValidateAndConvertProposedValue:function(/*out Collection<ProposedValue> values*/ parObj)
        {
//            Debug.Assert(NeedsValidation, "check NeedsValidation before calling this");
        	parObj.values = null; 

            // validate raw proposed value 
            var rawValue = this.GetRawProposedValue(); 

            var isValid = this.Validate(rawValue, ValidationStep.RawProposedValue); 

            // apply conversion and validate it
            var convertedValue = isValid ? ConvertProposedValue(rawValue)
                                            : DependencyProperty.UnsetValue; 

            if (convertedValue == EnsureBinding().DoNothing) 
            { 
                convertedValue = DependencyProperty.UnsetValue;
            } 
            else if (convertedValue == DependencyProperty.UnsetValue)
            {
                isValid = false;
            } 
            else
            { 
                isValid = this.Validate(convertedValue, ValidationStep.ConvertedProposedValue); 
            }
 
            // return the result
            parObj.values = new Collection/*<ProposedValue>*/();
            parObj.values.Add(new ProposedValue(this, rawValue, convertedValue));
            return isValid; 
        },
 
        /*private bool*/ 
        IsValidValueForUpdate:function(/*object*/ value, /*Type*/ sourceType) 
        {
            // null is always valid, even for value types.  The reflection layer 
            // apparently converts null to default(T).
            if (value == null)
                return true;
 
            // if direct assignment is possible, the value is valid
            if (sourceType.IsAssignableFrom(value.GetType())) 
                return true; 

            // if the value is DBNull, ask the worker (answer depends on several factors) 
            if (this.Convert.IsDBNull(value))
                return this.Worker.IsDBNullValidForUpdate;

            // otherwise the value is invalid 
            return false;
        }, 
 
        /*private void*/ 
        ProcessException:function(/*Exception*/ ex, /*bool*/ validate)
        { 
            var filteredException = null;
            /*ValidationError*/var validationError = null;

            // If there is not ExceptionFilter, then Wrap the 
            // exception in a ValidationError.
            if (this.ExceptionFilterExists()) 
            { 
                filteredException = this.CallDoFilterException(ex);
 
                if (filteredException == null)
                    return;

                validationError = filteredException instanceof ValidationError ? filteredException : null; 
            }
 
            // See if an ExceptionValidationRule is in effect 
            if (validationError == null && validate)
            { 
                /*ValidationRule*/var exceptionValidationRule = ExceptionValidationRule.Instance;

                if (filteredException == null)
                { 
                    validationError = new ValidationError(exceptionValidationRule, this, ex.Message, ex);
                } 
                else 
                {
                    validationError = new ValidationError(exceptionValidationRule, this, filteredException, ex); 
                }
            }

            if (validationError != null) 
            {
                this.UpdateValidationError(validationError); 
            } 
        },
 
        // Sometimes a source update gets requested as a result of event leapfrogging,
        // where the binding has not yet been informed of a pending change to its
        // path (e.g. Dev10 445406).  In this case, we should simply ignore the update.
        /*private bool*/ 
        ShouldIgnoreUpdate:function() 
        {
        	//cym add
        	EnsureComboBox();
        	
            // The detection algorithm below is expensive, so only do when the target 
            // property is especially susceptible to the problem 
            if (this.TargetProperty.OwnerType != EnsureSelecor().Type &&  // SelectedItem, SelectedValue, SelectedIndex, etc.
            		this.TargetProperty != (ComboBox != null ? ComboBox.TextProperty: null)                                           // ComboBox.Text 
                )
            {
                return false;
            } 

            // Re-evaluate the entire path.  If it doesn't produce the same source 
            // object as our cached value, there is presumably a change notification 
            // somewhere along the path that hasn't been delivered yet, due to event
            // leapfrogging. 

            // get initial (top-level) item
            var item;
            var contextElement = this.ContextElement; 
            if (contextElement == null)
            { 
                var target = this.TargetElement; 
                if (target != null && this.UsingMentor)
                { 
                    target = Helper.FindMentor(target);
                }
                if (target == null)
                { 
                    return true;
                } 
                var objRefArgs = new ObjectRefArgs();
                objRefArgs.ResolveNamesInTemplate = this.ResolveNamesInTemplate;
                item = this.ParentBinding.SourceReference.GetDataObject(target, 
                		objRefArgs);
            } 
            else
            {
                item = contextElement.GetValue(FrameworkElement.DataContextProperty);
            } 

            // Apply auto-magic rules for CVS, DSP, etc. 
            // This logic should agree with Activate(item), except for event hookup. 
            if (!this.ParentBinding.BindsDirectlyToSource)
            { 
                /*CollectionViewSource*/var cvs;
                /*DataSourceProvider*/var dsp;

                if ((cvs = (item instanceof CollectionViewSource ? item : null)) != null) 
                {
                    item = cvs.CollectionView; 
                } 
                else if ((dsp = (item instanceof DataSourceProvider ? item : null)) != null)
                { 
                    item = dsp.Data;
                }
            }
 
            // If the top-level item is different, ignore the update
            if (!Object.Equals(DataItem, item)) 
                return true; 

            // check the rest of the path 
            return !this.Worker.IsPathCurrent();
        },
 
        // fetch the current data errors from the source object (and its value), 
        // and update the list of data-error ValidationResults accordingly
        /*internal void*/ 
        UpdateNotifyDataErrors:function(/*INotifyDataErrorInfo*/ indei, /*string*/ propertyName, /*object*/ value) 
        {
            if (!this.ValidatesOnNotifyDataErrors || IsDetached)
                return;
 
            // replace the value object, if it has changed
            /*WeakReference*/
            var dataError = /*(WeakReference)*/GetFeatureValue(Feature.DataErrorValue, null); 
            /*INotifyDataErrorInfo*/
            var dataErrorValue = (dataError == null) ? null : dataError.isInstanceof(INotifyDataErrorInfo); 
            if (value != DependencyProperty.UnsetValue && value != dataErrorValue && this.IsDynamic)
            { 
                if (dataErrorValue != null)
                    ErrorsChangedEventManager.RemoveHandler(dataErrorValue, this.OnErrorsChanged);

                /*INotifyDataErrorInfo*/var newDataErrorValue = value instanceof INotifyDataErrorInfo ? value : null; 
// aaa               var newDataErrorWR = ReplaceReference(dataErrorValue, newDataErrorValue);
                this.SetFeatureValue(Feature.DataErrorValue, newDataError, null); 
                dataErrorValue = newDataErrorValue; 

                if (newDataErrorValue != null) 
                    ErrorsChangedEventManager.AddHandler(newDataErrorValue, this.OnErrorsChanged);
            }

            // fetch the errors from the last item and from its value 
            this.IsDataErrorsChangedPending = false;
            try 
            { 
                /*List<object> */var propertyErrors = BindingExpression.GetDataErrors(indei, propertyName);
                /*List<object>*/var valueErrors = BindingExpression.GetDataErrors(dataErrorValue, String.Empty); 
                /*List<object>*/var errors = this.MergeErrors(propertyErrors, valueErrors);

                this.UpdateNotifyDataErrorValidationErrors(errors);
            } 
            catch (/*Exception*/ ex)
            {   // if GetErrors or the enumerators throw, leave the old errors in place 
//                if (CriticalExceptions.IsCriticalApplicationException(ex)) 
//                    throw;
            } 
        },

        /*void*/ 
        UpdateNotifyDataErrors:function(/*object*/ value)
        { 
            if (!this.ValidatesOnNotifyDataErrors)
                return; 
 
            this.UpdateNotifyDataErrors(this.SourceItem instanceof INotifyDataErrorInfo ? this.SourceItem : null , this.SourcePropertyName, value);
        },

 
 
        /*List<object>*/ 
        MergeErrors:function(/*List<object>*/ list1, /*List<object>*/ list2) 
        {
            if (list1 == null) 
                return list2;
            if (list2 == null)
                return list1;
 
            for/*each*/ (var i=0; i<list2.Count; i++)
                list1.Add(list2.Get(i)); 
            return list1; 
        },
 
        /*private void*/ 
        OnDataContextChanged:function(/*DependencyObject*/ contextElement)
        { 
            // ADO BindingExpressions change the data context when a field changes. 
            // If the field is the one we're updating, ignore the DC change.
            if (!this.IsInUpdate && this.CanActivate) 
            {
                if (this.IsReflective && this.RootBindingExpression.ParentBindingBase.BindingGroupName == String.Empty)
                {
                	this.RejoinBindingGroup(this.IsReflective, contextElement); 
                }
 
                var newItem = contextElement.GetValue(FrameworkElement.DataContextProperty); 
                if (!Object.Equals(this.DataItem, newItem))
                { 
                	this.Activate(newItem);
                }
            }
        },

        /*internal void*/ 
        OnCurrentChanged:function(/*object*/ sender, /*EventArgs*/ e) 
        { 
        	this.Worker.OnCurrentChanged(sender instanceof ICollectionView ? sender : null, e);
        },

        /*internal void*/ 
        OnCurrentChanging:function(/*object*/ sender, /*CurrentChangingEventArgs*/ e)
        {
        	this.Update(); 
        }, 
 
        /*void*/ 
        OnDataChanged:function(/*object*/ sender, /*EventArgs*/ e)
        { 
        	this.Activate(sender); 
        },

        /*void*/ 
        OnInheritanceContextChanged:function(/*object*/ sender, /*EventArgs*/ e)
        { 
            if (this.StatusInternal == BindingStatusInternal.Unattached)
            { 
                // retry bindings immediately when InheritanceContext changes, 
                // so that triggers, animations, and rendering see the bound
                // value when they initialize their own local cache (bug DD 139838). 
            	this.Engine.CancelTask(this, TaskOps.AttachToContext);   // cancel existing task
            	this.AttachToContext(AttachAttempt.Again);

                if (this.StatusInternal == BindingStatusInternal.Unattached) 
                {
                    // if that didn't work, run the task again 
                    // By adding a new task, the engine will add a LayoutUpdated handler 
                    // (Dev11 4423)
                	this.Engine.AddTask(this, TaskOps.AttachToContext); 
                }
            }
            else
            { 
            	this.AttachToContext(AttachAttempt.Last);
            } 
        },

        /*internal override void*/ 
        OnLostFocus:function(/*object*/ sender, /*RoutedEventArgs*/ e) 
        {
        	this.Update();
        },
 
        /*void*/ OnErrorsChanged:function(/*object*/ sender, /*DataErrorsChangedEventArgs*/ e)
        { 
            // if notification was on the right thread, just do the work (normal case) 
//            if (Dispatcher.Thread == Thread.CurrentThread)
//            { 
        	this.UpdateNotifyDataErrors(DependencyProperty.UnsetValue);
//            }
//            else if (!IsDataErrorsChangedPending)
//            { 
//                // otherwise invoke an operation to do the work on the right context
//                IsDataErrorsChangedPending = true; 
//                Engine.Marshal( 
//                    (arg) => {  UpdateNotifyDataErrors(DependencyProperty.UnsetValue);
//                                 return null; }, null); 
//            }
        },

        /// <summary> 
        /// Handle events from the centralized event table
        /// </summary> 
        /*bool IWeakEventListener.*/
        ReceiveWeakEvent:function(/*Type*/ managerType, /*object*/ sender, /*EventArgs*/ e) 
        {
            return false;   // this method is no longer used (but must remain, for compat) 
        },
 
        //
        // If this BindingExpression's ParentBinding has an ExceptionFilter set, 
        // call it, otherwise give the MultiBinding (if there is one)
        // a chance.
        //
        /*private object*/ 
        CallDoFilterException:function(/*Exception*/ ex) 
        {
            if (this.ParentBinding.UpdateSourceExceptionFilter != null) 
            { 
                return this.ParentBinding.DoFilterException(this, ex);
            } 
            else if (this.IsInMultiBindingExpression)
            {
                return this.ParentMultiBindingExpression.ParentMultiBinding.DoFilterException(this, ex);
            } 

            return null; 
        }, 

        /*private bool*/ 
        ExceptionFilterExists:function() 
        {
            return ( (this.ParentBinding.UpdateSourceExceptionFilter != null) ||
                     (this.IsInMultiBindingExpression && this.ParentMultiBindingExpression.ParentMultiBinding.UpdateSourceExceptionFilter != null)
                   ); 
        },
 
 
        // surround any code that changes the value of a BindingExpression by
        //      using (bindExpr.ChangingValue()) 
        //      { ... }
        /*internal IDisposable*/ 
        ChangingValue:function()
        {
            return new ChangingValueHelper(this); 
        },
 
        // cancel any pending work 
        /*internal void*/ 
        CancelPendingTasks:function()
        { 
        	this.Engine.CancelTasks(this);
        },

        // replace this BindingExpression with a new one 
        /*void*/ 
        Replace:function()
        { 
            var target = this.TargetElement; 
            if (target != null)
            { 
                if (this.IsInBindingExpressionCollection)
                	this.ParentBindingExpressionBase.ReplaceChild(this);
                else
                    BindingOperations.SetBinding(target, this.TargetProperty, this.ParentBinding); 
            }
        }, 
        
   

        /*internal override void*/ 
        HandlePropertyInvalidation:function(/*DependencyObject*/ d, /*DependencyPropertyChangedEventArgs*/ args)
        {
            var dp = args.Property; 

            if (dp == FrameworkElement.DataContextProperty) 
            { 
                var contextElement = this.ContextElement;
                if (d == contextElement) 
                {
                	this.IsTransferPending = false;  // clear this flag, even if data context isn't changing (bug Dev11 162254)
                	this.OnDataContextChanged(contextElement);
                } 
            }
 
            if (dp == CollectionViewSource.ViewProperty) 
            {
                /*CollectionViewSource*/var cvs = this.CollectionViewSource; 
                if (d == cvs)
                {
                	this.Activate(cvs);
                } 
            }
 
            if (dp == FrameworkElement.LanguageProperty && this.UsesLanguage && d == this.TargetElement) 
            {
            	this.InvalidateCulture(); 
            	this.TransferValue();
            }

            if (this.Worker != null) 
            {
            	this.Worker.OnSourceInvalidation(d, dp, args.IsASubPropertyChange); 
            } 
        },
        
        
        /*void*/ 
        SetDataItem:function(/*object*/ newItem)
        {
//        	this._dataItem = this.CreateReference(newItem);
        	this._dataItem = newItem;
        }, 

        // find the DataSource object (if any) that produced the DataContext 
        // for element d 
        /*object*/ 
        GetDataSourceForDataContext:function(/*DependencyObject*/ d)
        { 
            // look for ancestor that contributed the inherited value
            var ancestor;
            var b = null;
 
            for (ancestor = d;
                 ancestor != null; 
                 ancestor = FrameworkElement.GetFrameworkParent(ancestor)) 
            {
                if (BindingExpression.HasLocalDataContext(ancestor)) 
                {
                	var b = BindingOperations.GetBindingExpression(ancestor, FrameworkElement.DataContextProperty);
                    b =  b instanceof BindingExpression ? b : null;
                    break;
                } 
            }
 
            if (b != null) 
                return b.DataSource;
 
            return null;
        },
        
 
//        void IDataBindEngineClient.
        VerifySourceReference:function(/*bool*/ lastChance) 
        {
            var target = this.TargetElement; 
            if (target == null)
                return;     // binding was detached since scheduling this task

            var sourceRef = this.ParentBinding.SourceReference; 
            var mentor = !UsingMentor ? target : Helper.FindMentor(target);
            /*ObjectRefArgs*/var args = new ObjectRefArgs();
//            { 
//                                    ResolveNamesInTemplate = ResolveNamesInTemplate, 
//                                    };
            args.ResolveNamesInTemplate = this.ResolveNamesInTemplate;
            var source = sourceRef.GetDataObject(mentor, args); 

            if (source != this.DataItem)
            {
                // the source reference resolves differently from before, so 
                // re-attach to the tree context
                this.AttachToContext(lastChance ? AttachAttempt.Last : AttachAttempt.Again); 
            } 
        }

	});

    // Create a new BindingExpression from the given Bind description 
    /*internal static BindingExpression*/ 
	BindingExpression.CreateBindingExpression = function(/*DependencyObject*/ d, 
                                            /*DependencyProperty*/ dp,
                                            /*Binding*/ binding, 
                                            /*BindingExpressionBase*/ parent)
    {
    	var temp =dp.GetMetadata(d.DependencyObjectType);
    	
    	 var fwMetaData = temp instanceof FrameworkPropertyMetadata ? temp : null;

        if ((fwMetaData != null && !fwMetaData.IsDataBindingAllowed) || dp.ReadOnly)
            throw new Error('ArgumentException(SR.Get(SRID.PropertyNotBindable, dp.Name), "dp")'); 

        // create the BindingExpression
        var bindExpr = new BindingExpression(binding, parent); 

        bindExpr.ResolvePropertyDefaultSettings(binding.Mode, binding.UpdateSourceTrigger, fwMetaData);

        // Two-way Binding with an empty path makes no sense 
        if (bindExpr.IsReflective && binding.XPath == null &&
                (binding.Path == null || String.IsNullOrEmpty(binding.Path.Path))) 
            throw new Error('InvalidOperationException(SR.Get(SRID.TwoWayBindingNeedsPath)'); 

        return bindExpr; 
    },
	
    // fetch errors for the given property
    /*internal static List<object>*/ 
    BindingExpression.GetDataErrors = function(/*INotifyDataErrorInfo*/ indei, /*string*/ propertyName)
    { 
        var RetryCount = 3;
        /*List<object>*/var result = null; 
        if (indei != null && indei.HasErrors) 
        {
            // if a worker thread is updating the source's errors while we're trying to 
            // read them, the enumerator will throw.   The interface doesn't provide
            // any way to work around this, so we'll just try it a few times hoping
            // for success.
            for (var i=RetryCount; i>=0; --i) 
            {
                try 
                { 
                    result = new List/*<object>*/();
                    /*IEnumerable*/var ie = indei.GetErrors(propertyName); 
                    if (ie != null)
                    {
//                        foreach (object o in ie)
//                        { 
//                            result.Add(o);
//                        } 
                        
                        var eu = ie.GetEnumerator();
                        while(eu.MoveNext()){
                        	result.Add(eu.Current);
                        }
                    } 
                    break;
                } 
                catch (/*InvalidOperationException*/ ex)
                {
                    // on the last try, let the exception bubble up
                    if (i == 0) 
                        throw ex;
                } 
            } 
        }

        if (result != null && result.Count == 0)
            result = null;

        return result; 
    };

    // raise the TargetUpdated event (explicit polymorphism)
    /*internal static void*/ 
    BindingExpression.OnTargetUpdated = function(/*DependencyObject*/ d, /*DependencyProperty*/ dp) 
    {
        /*DataTransferEventArgs*/var args = new DataTransferEventArgs(d, dp);
        args.RoutedEvent = EnsureBinding().TargetUpdatedEvent;
        /*FrameworkObject*/var fo = new FrameworkObject(d); 

        if (!fo.IsValid && d != null) 
        { 
            fo.Reset(Helper.FindMentor(d));
        } 

        fo.RaiseEvent(args);
    };

    // raise the SourceUpdatedEvent event (explicit polymorphism)
    /*internal static void*/ 
    BindingExpression.OnSourceUpdated = function(/*DependencyObject*/ d, /*DependencyProperty*/ dp) 
    { 
        /*DataTransferEventArgs*/var args = new DataTransferEventArgs(d, dp);
        args.RoutedEvent = EnsureBinding().SourceUpdatedEvent; 
        /*FrameworkObject*/var fo = new FrameworkObject(d);

        if (!fo.IsValid && d != null)
        { 
            fo.Reset(Helper.FindMentor(d));
        } 

        fo.RaiseEvent(args);
    };
    
    // return true if DataContext is set locally (not inherited) on DependencyObject 
    /*internal static bool*/ 
    BindingExpression.HasLocalDataContext = function(/*DependencyObject*/ d)
    { 
        var parObj = {"hasModifiers":null};
        var valueSource = d.GetValueSource(FrameworkElement.DataContextProperty, null, /*out hasModifiers*/parObj);
        var hasModifiers = parObj.hasModifiers;
        return (valueSource != BaseValueSourceInternal.Inherited) &&
                (valueSource != BaseValueSourceInternal.Default || hasModifiers); 
    };

	
	Object.defineProperties(BindingExpression.prototype,{
		
//		DependencyObject IDataBindEngineClient.
		TargetElement: 
        { 
            get:function() { 
            	return !this.UsingMentor ? /*TargetElement*/this._targetElement : Helper.FindMentor(/*TargetElement*/this._targetElement); 
            }
        }, 
 
        /// <summary> Binding from which this expression was created </summary>
        /*public Binding*/ 
		ParentBinding: { get:function() { return /*(Binding)*/this.ParentBindingBase; } }, 

        /// <summary>The data item actually used by this BindingExpression</summary>
        /*public object*/ 
		DataItem: { get:function() { return this._dataItem; } },
 
        /// <summary> The object that gets changed during UpdateSource </summary>
        /*public object*/ 
		ResolvedSource: { get:function() { return this.SourceItem; } }, 
 
        /// <summary> The name of the property that gets changed during UpdateSource </summary>
        /*public string*/ 
		ResolvedSourcePropertyName: { get:function() { return this.SourcePropertyName; } }, 

        /// <summary>The data source actually used by this BindingExpression</summary>
        /*internal object*/ 
		DataSource:
        { 
            get:function()
            { 
                var target = this.TargetElement; 
                if (target == null)
                    return null; 

                // if we're using DataContext, find the source for the DataContext
                if (this._ctxElement != null)
                    return GetDataSourceForDataContext(this.ContextElement); 

                // otherwise use the explicit source 
                /*ObjectRef*/var or = ParentBinding.SourceReference; 
                return or.GetObject(target, new ObjectRefArgs());
            } 
        },
        // The ContextElement is the DependencyObject (if any) whose DataContext 
        // is used as the starting point for the evaluation of the BindingExpression Path.
        //      We should not store a strong reference to the context element, 
        // for the same reasons as mentioned above for TargetElement.  Instead, 
        // we store a weak reference.  Callers should be prepared for the case
        // ContextElement==null, which is different from _ctxElement==null.  The 
        // former means the BindingExpression uses a context element, but that element has
        // been GC'd;  the latter means that the BindingExpression does not use a context
        // element.
        /*internal DependencyObject*/ 
        ContextElement: 
        {
            get:function() 
            { 
//                if (_ctxElement != null)
//                    return _ctxElement.Target as DependencyObject; 
//                else
//                    return null;
                
                if (this._ctxElement != null)
                    return this._ctxElement instanceof DependencyObject ? this._ctxElement : null; 
                else
                    return null;
            }
        }, 

        // The CollectionViewSource is the source object, as a CollectionViewSource. 
        /*internal CollectionViewSource*/ 
        CollectionViewSource: 
        {
            get:function() 
            {
//                WeakReference wr = (WeakReference)GetValue(Feature.CollectionViewSource, null);
//                return (wr == null) ? null : (CollectionViewSource)wr.Target;
            	return this.GetFeatureValue(Feature.CollectionViewSource, null);
            }, 
            set:function(value)
            { 
                if (value == null) 
                	this.ClearValue(Feature.CollectionViewSource);
                else 
//                	this.SetFeatureValue(Feature.CollectionViewSource, new WeakReference(value));
                	this.SetFeatureValue(Feature.CollectionViewSource, value);
            }
        },
 
        /// <summary> True if this binding expression should ignore changes from the source </summary>
        /*internal bool*/ 
        IgnoreSourcePropertyChange: 
        { 
            get:function()
            { 
                if (this.IsTransferPending || this.IsInUpdate)
                    return true;

                return false; 
            }
        }, 
 
        /*internal PropertyPath*/ 
        Path:
        { 
            get:function() { return this.ParentBinding.Path; }
        },

        /*internal IValueConverter*/ 
        Converter: 
        {
            get:function() { return /*(IValueConverter)*/this.GetFeatureValue(Feature.Converter, null); }, 
            set:function(value) { this.SetFeatureValue(Feature.Converter, value, null); } 
        },
 
        // MultiBinding looks at this to find out what type its MultiValueConverter should
        // convert back to, when this BindingExpression is not using a user-specified converter.
        /*internal Type*/ 
        ConverterSourceType:
        { 
            get:function() { return this._sourceType; }
        }, 
 
        // the item whose property changes when we UpdateSource
        /*internal object*/ 
        SourceItem: 
        {
            get:function() { return (this.Worker != null) ? this.Worker.SourceItem : null; }
        },
 
        // the name of the property that changes when we UpdateSource
        /*internal string*/ 
        SourcePropertyName: 
        { 
            get:function() { return (this.Worker != null) ? this.Worker.SourcePropertyName : null; }
        }, 

        // the value of the source property
        /*internal object*/ 
        SourceValue:
        { 
            get:function() { return (this.Worker != null) ? this.Worker.RawValue() : DependencyProperty.UnsetValue; }
        }, 
 
        /*internal override bool*/ 
        IsParentBindingUpdateTriggerDefault:
        { 
            get:function() { return (this.ParentBinding.UpdateSourceTrigger == UpdateSourceTrigger.Default); }
        },

        /*internal override bool*/ 
        IsDisconnected: 
        {
            get:function() { return this._dataItem === BindingExpressionBase.DisconnectedItem; } 
        },
        
        /*private bool*/ 
        CanActivate:
        {
            get:function() { return this.StatusInternal != BindingStatusInternal.Unattached; } 
        }, 

        /*private BindingWorker*/ 
        Worker: { get:function() { return this._worker; } },

        /*private DynamicValueConverter*/ 
        DynamicConverter:
        {
            get:function() 
            {
                if (!this.HasValue(Feature.DynamicConverter)) 
                { 
//                    Invariant.Assert(Worker != null);
                    // pass along the static source and target types to find same DefaultValueConverter as SetupDefaultValueConverter 
                    this.SetFeatureValue(Feature.DynamicConverter, 
                    		new DynamicValueConverter(this.IsReflective, this.Worker.SourcePropertyType, 
                    				this.Worker.TargetPropertyType), null);
                }
                return /*(DynamicValueConverter)*/this.GetFeatureValue(Feature.DynamicConverter, null);
            } 
        },
 
        /*private DataSourceProvider*/ 
        DataProvider:
        {
            get:function() { return /*(DataSourceProvider)*/this.GetFeatureValue(Feature.DataProvider, null); }, 
            set:function(value) { this.SetFeatureValue(Feature.DataProvider, value, null); }
        }
	});
	
	BindingExpression.NullDataItem = Type.NullDataItem ;//{"name" : "NullDataItem"};
	BindingExpression.IgnoreDefaultValue = Type.IgnoreDefaultValue; //{"name" : "IgnoreDefaultValue"}; 
	BindingExpression.StaticSource = Type.StaticSource; //{"name" : "StaticSource"};
	
	BindingExpression.Type = new Type("BindingExpression", BindingExpression, [BindingExpressionBase.Type, IDataBindEngineClient.Type, IWeakEventListener.Type]);
	return BindingExpression;
});