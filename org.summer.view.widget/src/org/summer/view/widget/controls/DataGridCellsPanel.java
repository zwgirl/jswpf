...
//    #region Measure 
    ...
    { 
        ...
        boolean isColumnHeader = (child is DataGridColumnHeader); 
    ...
    private Size DetermineRealizedColumnsBlockList(Size constraint) 
    {
        ...
        boolean hasStarColumns = parentDataGrid.InternalColumns.HasVisibleStarColumns;
        double averageColumnWidth = parentDataGrid.InternalColumns.AverageColumnWidth; 
        boolean invalidAverage = DoubleUtil.AreClose(averageColumnWidth, 0.0);
        boolean notVirtualizing = !IsVirtualizing;
        boolean generateAll = invalidAverage || hasStarColumns || notVirtualizing;
        int frozenColumnCount = parentDataGrid.FrozenColumnCount; 
        int previousColumnIndex = -1;

        boolean redeterminationNeeded = false; 
        Size childSize;
        ...
        {
            boolean isColumnHeader = ParentPresenter is DataGridColumnHeadersPresenter; 
            if (isColumnHeader)
            { 
    ...
    /// </summary>
    private void AddContainerFromGenerator(int childIndex, UIElement child, boolean newlyRealized) 
    { 
        if (!newlyRealized)
        { 
    ...
    /// </summary> 
    private void InsertContainer(int childIndex, UIElement container, boolean isRecycled) 
    {
        ...
        { 
            if (visualTreeIndex < children.Count)
            {
                ...
                if (isRecycled && VisualTreeHelper.GetParent(container) != null) 
                {
                    ...
                    // insert it in the proper location.   We cant use an /*internal*/ public Move api, so we are removing 
    ...
    /// </summary>
    private static boolean InBlockOrNextBlock(List<RealizedColumnsBlock> blockList, int index, ref int blockIndex, ref RealizedColumnsBlock block, out boolean pastLastBlock) 
    {
        pastLastBlock = false;
        boolean exists = true;
    ...
    /// </summary>
    private boolean GenerateChildForFocusTrail(
    ...
    private void VirtualizeChildren(List<RealizedColumnsBlock> blockList, IItemContainerGenerator generator) 
    {
        ...
        boolean pastLastBlock = (blockCount > 0 ? false : true);
        ...
        { 
            ...
            boolean virtualizeChild = pastLastBlock || !InBlockOrNextBlock(blockList, columnIndex, ref blockIndex, ref block, out pastLastBlock);
    ...
    protected /*override*/ Size ArrangeOverride(Size arrangeSize)
    {
        ...
        if (displayIndexBlockList != null && displayIndexBlockList.Count > 0)
        {
            ...
            boolean pastLastBlock = false; 
            for (int i = 0, count = parentDataGrid.Columns.Count; i < count; i++)
            { 
                boolean realizedChild = InBlockOrNextBlock(displayIndexBlockList, i, ref displayIndexBlockIndex, ref displayIndexBlock, out pastLastBlock); 
                if (pastLastBlock)
                { 
                    break;
                }
    ...
    /*internal*/ public boolean HasCorrectRealizedColumns
    {
        get
        { 
    ...
    private boolean RebuildRealizedColumnsBlockList 
    {
        get
        {
            DataGrid dataGrid = ParentDataGrid; 
    ...
    { 
        super.OnIsItemsHostChanged(oldIsItemsHost, newIsItemsHost);
    ...
    /// </summary>
    /*internal*/ public double ComputeCellsPanelHorizontalOffset() 
    {
    ...
    /*internal*/ public void InternalBringIndexIntoView(int index)
    { 
    ...
    protected /*internal*/ /*override*/ void BringIndexIntoView(int index) 
    {
        ...
        if (parentDataGrid == null) 
        {
            super.BringIndexIntoView(index); 
            return; 
        }
    ...
    /// </summary> 
    private boolean IsChildInView(int index, /*out*/ double newHorizontalOffset)
    { 
        DataGrid parentDataGrid = ParentDataGrid;
        double horizontalOffset = parentDataGrid.HorizontalScrollOffset;
...
//    #region Frozen Columns
    ...
    /// <returns></returns>
    /*internal*/ public Geometry GetFrozenClipForChild(UIElement child) 
    { 
        if (child == _clippedChildForFrozenBehaviour)
        { 
