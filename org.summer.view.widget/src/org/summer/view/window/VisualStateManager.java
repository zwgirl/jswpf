...
import org.summer.view.widget.VisualState;
import org.summer.view.widget.VisualStateGroup;
import org.summer.view.widget.VisualTransition;
...
import org.summer.view.widget.media.animation.Timeline;
...
public class VisualStateManager extends DependencyObject
{
    ...
    private static boolean GoToStateCommon(FrameworkElement control, FrameworkElement stateGroupsRoot, String stateName, boolean useTransitions)
    { 
        if (stateName == null)
        { 
    ...
    /// <param name="useTransitions">Whether to use transition animations.</param>
    /// <returns>true if the state changed successfully, false otherwise.</returns>
    public static boolean GoToState(FrameworkElement control, String stateName, boolean useTransitions)
    { 
        if (control == null)
        { 
    ...
    /// <param name="useTransitions">Whether to use transition animations.</param>
    /// <returns>true if the state changed successfully, false otherwise.</returns>
    public static boolean GoToElementState(FrameworkElement stateGroupsRoot, String stateName, boolean useTransitions) 
    {
        if (stateGroupsRoot == null) 
        { 
    ...
    /// </summary> 
    protected /*virtual*/ boolean GoToStateCore(FrameworkElement control, FrameworkElement stateGroupsRoot, String stateName, VisualStateGroup group, VisualState state, boolean useTransitions)
    { 
        return GoToStateInternal(control, stateGroupsRoot, group, state, useTransitions);
    }
...
//    #region CustomVisualStateManager 
    ...
    public static final DependencyProperty CustomVisualStateManagerProperty = 
...
//    #region VisualStateGroups
    ...
    private static final DependencyPropertyKey VisualStateGroupsPropertyKey =
    ...
    /// </summary>
    public static final DependencyProperty VisualStateGroupsProperty =
            VisualStateGroupsPropertyKey.DependencyProperty;

    /*internal*/ public static Collection<VisualStateGroup> GetVisualStateGroupsInternal(FrameworkElement obj)
    { 
        if (obj == null) 
        {
            throw new ArgumentNullException("obj"); 
...
//    #region State Change 
    ...
    /*internal*/ public static boolean TryGetState(IList<VisualStateGroup> groups, String stateName, /*out*/ VisualStateGroup group, /*out*/ VisualState state)
    {
    ...
    private static boolean GoToStateInternal(FrameworkElement control, FrameworkElement stateGroupsRoot, VisualStateGroup group, VisualState state, boolean useTransitions)
    { 
        if (stateGroupsRoot == null)
        { 
            throw new ArgumentNullException("stateGroupsRoot"); 
...
//    [SecurityCritical, SecurityTreatAsSafe] 
    private static boolean ShouldRunStateStoryboard(FrameworkElement control, FrameworkElement stateGroupsRoot, VisualState state, VisualStateGroup group)
    {
        boolean controlInTree = true;
        boolean stateGroupsRootInTree = true; 
...
//    #region Transitions 
    ...
    private static Storyboard GenerateDynamicTransitionAnimations(FrameworkElement root, VisualStateGroup group, VisualState newState, VisualTransition transition)
    {
        ...
        for/*each*/ (KeyValuePair<TimelineDataToken, Timeline> pair : currentAnimations)
        { 
    ...
    { 
        ...
        {
            result = new ColorAnimation(); // { EasingFunction = easingFunction };  cym comment
        }
        else if (timeline is DoubleAnimation || timeline is DoubleAnimationUsingKeyFrames)
        {
            result = new DoubleAnimation(); // { EasingFunction = easingFunction }; cym comment
        }
        else if (timeline is PointAnimation || timeline is PointAnimationUsingKeyFrames) 
        { 
            result = new PointAnimation(); // { EasingFunction = easingFunction };cym comment
        } 
    ...
    private static Timeline GenerateToAnimation(FrameworkElement root, Timeline timeline, IEasingFunction easingFunction, boolean isEntering)
    { 
        Timeline result = null;
    ...
    /// </returns>
    /*internal*/ public static VisualTransition GetTransition(FrameworkElement element, VisualStateGroup group, VisualState from, VisualState to)
    {
        if (element == null) 
        {
...
//    #region GetTarget Methods 
    ...
    private static Color/*?*/ GetTargetColor(Timeline timeline, boolean isEntering)
    { 
    ...
    private static double/*?*/ GetTargetDouble(Timeline timeline, boolean isEntering)
    { 
    ...
    private static Point/*?*/ GetTargetPoint(Timeline timeline, boolean isEntering) 
    {
...
//    #region FlattenTimelines
    ...
    private class TimelineDataToken implements IEquatable<TimelineDataToken> 
    { 
        ...
        public boolean Equals(TimelineDataToken other) 
        { 
            boolean targetsEqual = false;
            if (_targetName != null) 
            {
...
//    #region Data
    ...
    private static final Duration DurationZero = new Duration(TimeSpan.Zero);
